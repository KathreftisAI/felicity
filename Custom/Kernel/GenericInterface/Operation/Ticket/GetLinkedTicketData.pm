...
package Kernel::GenericInterface::Operation::Ticket::GetLinkedTicketData;
...
sub Run {
    ...
    return $Self->ReturnError(
        ErrorCode    => 'GetLinkedTicketData.AuthFail',
        ErrorMessage => "GetLinkedTicketData: Authorization failing!",
    ) if !$UserID;
    ...
    my $TicketCountSQL
        = "Select count(source_key) from link_relation where (source_key=? or target_key=?) AND (target_object_id=1 AND source_object_id=1)";
    return
        if !$DBObject->Prepare(
        SQL  => $TicketCountSQL,
