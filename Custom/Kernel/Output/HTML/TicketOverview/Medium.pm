...
sub Run {
    ...
    if ( scalar @{ $Param{TicketIDs} } ) {
        ...
        for my $TicketID ( @{ $Param{TicketIDs} } ) {
            ...
            {
            # Start of Code to get attachment count  & Article Count 
            # GetArticleList
            @ArticleBoxAtm = $Kernel::OM->Get('Kernel::System::Ticket')->ArticleContentIndex(
                TicketID                   => $TicketID,
            ...
            my $ArticleCounter = 0;
            for my $ArticleAtmCounter (@ArticleBoxAtm){
                my $c = $ArticleAtmCounter->{Atms};
                $ArticleCounter++;
                $AtmCount{$TicketID} += int(keys %{$c});
            }
            $ArticleCount{$TicketID}=$ArticleCounter;

            # End of Code to get attachment count & Article Count
                ...
                my $Output = $Self->_Show(
                    ...
                    Bulk     => $BulkFeature,
                    Config   => $Param{Config},
                    AtmCount => $AtmCount{$TicketID},
                    ArticleCount => $ArticleCount{$TicketID},
...
sub _Show {
    ...
    my $AtmCount = $Param{AtmCount};
    my $ArticleCount = $Param{ArticleCount};
...
# ---
    ...
    for my $DynamicFieldConfig ( @{$DynamicField} ) {
        ...
        #Code Start To show PrintStatus ,ArticleCount & AttachmentCount on Grid
        if ($DynamicFieldConfig->{Name} eq 'PrintStatus' && $Value ne 'Printed'){
            $Value='Not Printed';
        }

        if ($DynamicFieldConfig->{Name} eq 'AttachmentCount'){
            $Value=$AtmCount;
        }

        if ($DynamicFieldConfig->{Name} eq 'ArticleCount'){
            $Value=$ArticleCount;
        }
        #Code End To show PrintStatus ,ArticleCount & AttachmentCount on Grid
