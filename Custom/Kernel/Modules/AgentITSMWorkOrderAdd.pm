...
sub Run {
    ...
    my %GetParam;
    for my $ParamName (
        qw(WorkOrderTitle Instruction WorkOrderTypeID PlannedEffort AttachmentUpload FileID AgentType UserSelected Customer CustomerSelected)
        )
    {
    ...
    if ( $Self->{Subaction} eq 'Save' ) {
        ...
        my $WorkOrderUserType = $GetParam{AgentType};
            ...
            if ($WorkOrderID) {
                ...
                if($WorkOrderUserType eq 'Agent'){
                    ...
                    if(!$Success){
                        $LogObject->Log(
                            Priority => 'error',
                            Message  => "Could not update WorkOrder Agent "
                                            . "for WorkOrderID '$WorkOrderID'!",
                        );
                    }

                } elsif ($WorkOrderUserType eq 'Customer') {

                    my $CustomerITSMWorkOrderObject = $Kernel::OM->Get('Kernel::System::CustomerITSMWorkOrder');

                    my $CustomerWorkOrderID = $CustomerITSMWorkOrderObject->CustomerWorkOrderAdd(
                        ChangeID        => $ChangeID,
                        WorkOrderID     => $WorkOrderID,
                        CustomerLogin   => $GetParam{CustomerSelected},
                        UserID          => $Self->{UserID},
                    );
                    ...
                    if(!$CustomerWorkOrderID){               
                        ...
                        return $LayoutObject->ErrorScreen(
                            Message => Translatable('Was not able to add workorder to customer!'),
                            Comment => Translatable('Please contact the admin.'),
                        ); 
                    }

                    my $WorkOrderCustomerConfig = $Kernel::OM->Get('Kernel::System::DynamicField')->DynamicFieldGet(
                        Name   => 'WorkOrderCustomer',            #WorkOrderCustomer DynamicField of type WorkOrder to store Assigned Customer       
                    );

                    # set the value
                    my $Success = $DynamicFieldBackendObject->ValueSet(
                        DynamicFieldConfig => $WorkOrderCustomerConfig,
                        ObjectID           => $WorkOrderID,
                        Value              => $GetParam{Customer},
                        UserID             => $Self->{UserID},
                    );
                }
