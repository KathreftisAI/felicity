...
sub Run {
    ...
    ######## Start of Code for DynamicField Tagging By Milan######################
    my $ComposeConfig=$ConfigObject->Get("DynamicField_Tagging::Default::AgentTicketCompose");
    my %ComposeConfigHash= %{$ComposeConfig};

    my $ComposeMandatoryConfig=$ConfigObject->Get("DynamicField_Tagging::Default::AgentTicketCompose-Mandatory");
    my %ComposeMandatoryConfigHash= %{$ComposeMandatoryConfig};

    for my $Keys (keys %ComposeConfigHash){
        if($Ticket{Queue} eq $Keys || $Ticket{QueueID} eq $Keys){
            my @QueueDynamicField = split(',',$ComposeConfigHash{$Keys});
            for my $Dynamickeys(0 .. $#QueueDynamicField){ 
                $Config->{DynamicField}->{$QueueDynamicField[$Dynamickeys]}='1';
            }    
        }
    }

    for my $Keys (keys %ComposeMandatoryConfigHash){
        if($Ticket{Queue} eq $Keys || $Ticket{QueueID} eq $Keys){
            my @QueueDynamicField = split(',',$ComposeMandatoryConfigHash{$Keys});
            for my $Dynamickeys(0 .. $#QueueDynamicField){
                $Config->{DynamicField}->{$QueueDynamicField[$Dynamickeys]}='2';
            }
        }
    }
    ######## End of Code for DynamicField Tagging ######################
