...
sub Run {
    ...
    my $CustomerWorkOrderExist = $CustomerITSMWorkOrderObject->CustomerWorkOrderExist(
        WorkOrderID => $WorkOrderID,
        UserID      => $Self->{UserID},
    );


    if ($CustomerWorkOrderExist) {
        my $CustomerWorkOrder = $CustomerITSMWorkOrderObject->CustomerWorkOrderGet(
            WorkOrderID => $WorkOrderID,
            UserID      => $Self->{UserID},
        );

        $WorkOrder->{CustomerName} = $CustomerUserObject->CustomerName(
            UserLogin => $CustomerWorkOrder->{CustomerLogin},
        );
        
        $WorkOrder->{CustomerName}= "$CustomerWorkOrder->{CustomerLogin} ($WorkOrder->{CustomerName})";
