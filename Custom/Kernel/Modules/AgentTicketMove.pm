...
sub Run {
    ...
    my $MoveConfig=$ConfigObject->Get("DynamicField_Tagging::Default::AgentTicketMove");
    my %MoveConfigHash= %{$MoveConfig};

    my $MoveMandatoryConfig=$ConfigObject->Get("DynamicField_Tagging::Default::AgentTicketMove-Mandatory");
    my %MoveMandatoryConfigHash= %{$MoveMandatoryConfig};

    for my $Keys (keys %MoveConfigHash){
        if($Ticket{Queue} eq $Keys || $Ticket{QueueID} eq $Keys){
            my @QueueDynamicField = split(',',$MoveConfigHash{$Keys});
    ...
    for my $Keys (keys %MoveMandatoryConfigHash){
        if($Ticket{Queue} eq $Keys || $Ticket{QueueID} eq $Keys){
            my @QueueDynamicField = split(',',$MoveMandatoryConfigHash{$Keys});
            for my $Dynamickeys(0 .. $#QueueDynamicField){
