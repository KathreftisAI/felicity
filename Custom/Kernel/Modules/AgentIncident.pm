...
package Kernel::Modules::AgentIncident;
...
sub Run {
    ...
    COLUMNNAME:
    for my $ColumnName (
        qw(Owner Responsible State Queue Priority Lock Service SLA CustomerID CustomerUserID)
        )
    {
	...
	# for the fileter removing 
    my @ViewableTypeIDs =(2);
    ...
    # close states
    my @CloseStateIDs = $Kernel::OM->Get('Kernel::System::State')->StateGetStatesByType(
        StateType   => ['closed'],
        Result => 'ID',
    );
    ...
    my @ViewableQueueIDs;
    if ( !$Self->{QueueID} ) {
        #added by milan to get all Queues List
        @ViewableQueueIDs = $Self->AllQueuesList();
        $Self->{ShowAllTicketFlag}= 1;
    }
    else {
    ...
    my %Filters = (
		...
		TypeIDs       => \@ViewableTypeIDs,
            },
        },

        #added for show close ticket on incident overview page by ashish
        Close => {
            Name   => Translatable('Close tickets'),
            Prio   => 1002,
            Search => {
                StateIDs => \@CloseStateIDs,
                QueueIDs => \@ViewableQueueIDs,
    ...
    #To remove column state filter while ticket search on close
    my %ColumnFilterSearch = %ColumnFilter;
    if ($Filter eq 'Close' && $ColumnFilter{StateIDs}) {
        delete $ColumnFilterSearch{StateIDs};
    }
    ...
    for my $FilterColumn ( sort keys %Filters ) {
        my $Count = 0;
        if (@ViewableQueueIDs) {
            #To remove state column filter in close tickets while counting tickets
            my %ColumnFilterCount=%ColumnFilter;
            if ($FilterColumn eq 'Close' && $ColumnFilter{StateIDs}) {
                delete $ColumnFilterCount{StateIDs};
            }
...
sub _MaskQueueView {
    ...
    #variable added by milan for dropdown
    my $SelectedQueue;
    ...
    #Variable for QueueDropdown
    my %DropdownQueueList;
...
#################End of code#################################################        
        ...
        $QueueStrg
            .= "<li><a href=\"$LayoutObject->{Baselink}Action=AgentIncident;QueueID=$Queue{QueueID}";
        $QueueStrg .= ';View=' . $LayoutObject->Ascii2Html( Text => $View );
        $QueueStrg .= ';Filter=' . $LayoutObject->Ascii2Html( Text => $Filter );
        if ($Param{UseSubQueues} && $QueueID eq $Queue{QueueID} && $Config->{UseSubQueues} eq $Param{UseSubQueues} ) {
        ...
        elsif ( $Self->{HighlightAge2} && $Queue{MaxAge} >= $Self->{HighlightAge2} ) {
            $QueueStrg .= 'OlderLevel2';
        }
        elsif ( $Self->{HighlightAge1} && $Queue{MaxAge} >= $Self->{HighlightAge1} ) {
		...
		## Commented by Geeta Suryawanshi ~GS
...
################################################################
    ...
    return (
        MainName      => 'Queues',
        DropdownQueueList => $Param{DropdownQueueList},#line added for Queue dropdown
...
sub AllQueuesList{
    ...
    my $QueueID         = $Param{QueueID} || 0;
    my @QueuesNew       = @{ $Data{Queues} };
    my $QueueIDOfMaxAge = $Data{QueueIDOfMaxAge} || -1;
    my %AllQueues       = $Kernel::OM->Get('Kernel::System::Queue')->QueueList( Valid => 0 );
    my %Counter;
    my %Totals;
    my $HaveTotals = 0;    # flag for "Total" in index backend
    ...
    my @ListOfQueues;
    for my $Keys (@ListedQueues){
        push @ListOfQueues , $Keys->{QueueID};
    }
