...
sub Run {
    ...
    my $StateID     = $ParamObject->GetParam( Param => 'WorkOrderStateID' );
    ...
    $Self->{Config}
        = $ConfigObject->Get("ITSMWorkOrder::Frontend::$Self->{Action}");
    ...
    if ( !$WorkOrder ) {
        return $LayoutObject->ErrorScreen(
            Message => $LayoutObject->{LanguageObject}->Translate(
                'WorkOrder "%s" not found in database!', $WorkOrderID
            ),
            Comment => Translatable('Please contact the admin.'),
        );
    }
    ...
    # if ($StateID) {
    #     $GetParam{'WorkOrderStateID'} = $StateID;
    # }

    for my $ParamName (
        qw(Report WorkOrderStateID AccountedTime AttachmentUpload FileID))
    {
        $GetParam{$ParamName} = $ParamObject->GetParam( Param => $ParamName );
    }

    my $WorkOrderTypeDynamicFieldConfig = $ConfigObject->Get(
        "ITSMWorkOrder::Frontend::AgentITSMWorkOrderType::DynamicField");
    my %WorkOrderTypeDynamicFieldConfigHash
        = %{$WorkOrderTypeDynamicFieldConfig};

    for my $keys ( keys %WorkOrderTypeDynamicFieldConfigHash ) {
        if ( $WorkOrder->{WorkOrderTypeID} eq $keys ) {
            my @TypeDynamicFields
                = split( ',', $WorkOrderTypeDynamicFieldConfigHash{$keys} );
            for my $DynamicKeys ( 0 .. $#TypeDynamicFields ) {
                $Self->{Config}->{DynamicField}
                    ->{ $TypeDynamicFields[$DynamicKeys] } = '1';
            }
        }
    }
...
ATTACHMENT:
    for my $Number ( 1 .. 32 ) {
        ...
        my $Delete
            = $ParamObject->GetParam( Param => "AttachmentDelete$Number" );
    ...
    if ( $Self->{Subaction} eq 'Save' ) {
        ...
        # transform work order actual time, time stamp based on user time zone
                    %GetParam
                        = $LayoutObject->TransformDateSelection( %GetParam,
                        Prefix => $TimeType, );
              ...
              # format as timestamp, when all required time params were passed
                    ...
                    $ValidationError{ $TimeType . 'ErrorType' }
                        = 'GenericServerError';
                }
            }
            ...
            if ( $SystemTime{ActualEndTime} && !$SystemTime{ActualStartTime} )
            {
                ...
                && ( $SystemTime{ActualEndTime}
                    < $SystemTime{ActualStartTime} )
                )
            {
                $ValidationError{ActualStartTimeInvalid} = 'ServerError';
                $ValidationError{ActualStartTimeErrorType}
                    = 'BeforeThanEndTimeServerError';
            }
        }
...
# propagate validation error to the Error variable to be detected by the frontend
        ...
        if ( !%ValidationError ) {
            ...
            if ($CouldUpdateWorkOrder) {
                ...
                my @ExistingAttachments
                    = $WorkOrderObject->WorkOrderReportAttachmentList(
                    WorkOrderID => $WorkOrderID, );
            ...
            FILENAME:
                for my $Filename (@ExistingAttachments) {
                    ...
                    my $AttachmentData
                        = $WorkOrderObject->WorkOrderAttachmentGet(
                        WorkOrderID    => $WorkOrderID,
                        Filename       => $Filename,
                    ...
                    next FILENAME
                        if $AttachmentData->{Preferences}->{ContentID};
...
# attachment is already existing, we can delete it from the new attachment hash
                    ...
                    # existing attachment is no longer in new attachments hash
                        ...
                        my $DeleteSuccessful
                            = $WorkOrderObject->WorkOrderAttachmentDelete(
            ...
            ATTACHMENT:
                ...
                return $LayoutObject->PopupClose( URL =>
                        "Action=AgentITSMWorkOrderZoom;WorkOrderID=$WorkOrderID",
                );
            }
            else {
    ...
    if ( !$Change ) {
        return $LayoutObject->ErrorScreen(
            Message => $LayoutObject->{LanguageObject}->Translate(
                'Could not find Change for WorkOrder %s!', $WorkOrderID
            ),
            Comment => Translatable('Please contact the admin.'),
        );
    }
    ...
    my $WorkOrderPossibleStates
        = $WorkOrderObject->WorkOrderPossibleStatesGet(
        WorkOrderID => $WorkOrderID,
        UserID      => $Self->{UserID},
        );
...
# get workorder dynamic fields from workorder if page is loaded the first time
    ...
    if ( $Self->{Config}->{ActualTimeSpan} ) {
        ...
        for my $TimeType (qw(ActualEndTime ActualStartTime)) {
            ...
            my %TimePeriod
                = %{ $ConfigObject->Get('ITSMWorkOrder::TimePeriod') };
            ...
            my $TimeSelectionString = $LayoutObject->BuildDateSelection(
                ...
                "${TimeType}Optional" => 1,
                $TimeType
                    . 'Class' => $ValidationError{ $TimeType . 'Invalid' }
                    || '',
                Validate => 1,
                %TimePeriod,
            );
