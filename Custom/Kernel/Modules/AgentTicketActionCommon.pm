...
sub Run {
    ...
    my $TaggingConfig = $ConfigObject->Get("DynamicField_Tagging::Default::$Self->{Action}");
    my %TaggingConfigHash = %{$TaggingConfig};

    my $TaggingMandatoryConfig = $ConfigObject->Get("DynamicField_Tagging::Default::$Self->{Action}-Mandatory");
    my %TaggingMandatoryConfigHash = %{$TaggingMandatoryConfig};

    if(%TaggingConfigHash){
        for my $Keys (keys %TaggingConfigHash){
            if($Ticket{Queue} eq $Keys || $Ticket{QueueID} eq $Keys){
                my @QueueDynamicField = split(',',$TaggingConfigHash{$Keys});
    ...
    if(%TaggingMandatoryConfigHash){
        for my $Keys (keys %TaggingMandatoryConfigHash){
            if($Ticket{Queue} eq $Keys || $Ticket{QueueID} eq $Keys){
                my @QueueDynamicField = split(',',$TaggingMandatoryConfigHash{$Keys});
                for my $Dynamickeys(0 .. $#QueueDynamicField){
