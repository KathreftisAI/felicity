...
package Kernel::Modules::AgentCatalog;
use strict;
use warnings;
...
sub Run {
    ...
    if ($Self->{Subaction} eq 'Request') {
        ...
        my $TicketID = $TicketObject->TicketCreate(
            ...
            QueueID        => $ItemData{Queue_id},            # or QueueID => 123,
            Lock         => 'unlock',
        ...
        my $ArticleID = $TicketObject->ArticleCreate(
            ...
            From             => $ConfigObject->Get('AdminEmail') || 'otrs@localhost',       # not required but useful
            To               => "$Self->{UserFirstname} $Self->{UserLastname}  <$Self->{UserEmail}>", # not required but useful
            ...
            HistoryComment   => 'Request for '.$ItemName,
            UserID           => $Self->{UserID},
