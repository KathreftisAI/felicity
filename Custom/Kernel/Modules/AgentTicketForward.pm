...
###############################################################################
#--------start of code for mapping dynamic fields based on QueueID or Queue ----By Rohit-Ambre
###############################################################################

    #Get config for queue to dynamic field tagging
    my $ForwardConfig = $ConfigObject->Get("DynamicField_Tagging::Default::AgentTicketForward");
    my %ForwardConfigHash= %{$ForwardConfig};
    
    #Get config for queue to mandatory dynamic field tagging
    my $ForwardMandatoryConfig = $ConfigObject->Get("DynamicField_Tagging::Default::AgentTicketForward-Mandatory");
    my %ForwardMandatoryConfigHash= %{$ForwardMandatoryConfig};

    for my $keys (keys %ForwardConfigHash){
        if( $Ticket{Queue} eq $keys || $Ticket{QueueID} eq $keys){
            my @QueueDynamicFields = split(',',$ForwardConfigHash{$keys});
            for my $DynamicKeys (0 .. $#QueueDynamicFields){
                $Config->{DynamicField}->{$QueueDynamicFields[$DynamicKeys]} = '1';
            }
        }
    }
    
    for my $keys (keys %ForwardMandatoryConfigHash){
        if( $Ticket{Queue} eq $keys || $Ticket{QueueID} eq $keys){
            my @QueueDynamicFields = split(',',$ForwardMandatoryConfigHash{$keys});
            for my $DynamicKeys (0 .. $#QueueDynamicFields){
                $Config->{DynamicField}->{$QueueDynamicFields[$DynamicKeys]} = '2';
            }
        }
    }
######################################################################################
#-------------END of Code by Rohit------------------
######################################################################################
    ...
    #Get User Signature or System signature
    my $TemplateGenerator = $Kernel::OM->Get('Kernel::System::TemplateGenerator');
    my $UserObject = $Kernel::OM->Get('Kernel::System::User');
    my %Preferences = $UserObject->GetPreferences(
    UserID => $Self->{UserID},
    );
    if(%Preferences->{SignatureValidID} eq '2'){
        $Data{Signature}=%Preferences->{Signature};
    }
    else{
