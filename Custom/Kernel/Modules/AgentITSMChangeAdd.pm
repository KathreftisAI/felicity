...
sub Run {
    ...
    my %GetParam;
    for my $ParamName (
        qw(ChangeTitle Description Justification TicketID CategoryID ImpactID PriorityID AttachmentUpload FileID RequireAndOr WorkorderTagging NoofApprovers ITSMChangeGroupID)
        )
    {
    ...
    my $DynamicFieldWorkOrderTagging
        = $DynamicFieldObject->DynamicFieldGet( Name => 'WorkorderTagging', );
...
DYNAMICFIELD:
    ...
    if ( $Self->{Config}->{RequestedTime} ) {
        for my $TimePart (qw(Used Year Month Day Hour Minute)) {
            ...
            $GetParam{$ParamName}
                = $ParamObject->GetParam( Param => $ParamName );
        }
    }
    ...
    if ( !$Param{CategoryID} ) {
        my $DefaultCategory
            = $ConfigObject->Get('ITSMChange::Category::Default');
    ...
    if ( $GetParam{TicketID} ) {
        ...
        my $AddChangeLinkTicketTypes
            = $ConfigObject->Get('ITSMChange::AddChangeLinkTicketTypes');

        # check the list of relevant ticket types
        if (   !$AddChangeLinkTicketTypes
    ...
    my $CIPAllocateObject = $Kernel::OM->Get(
    ...
    if ( $Self->{Subaction} eq 'Save' ) {
        ...
        for my $Type (qw(Category Impact Priority)) {
                ...
                || $GetParam{"${Type}ID"} !~ m{ \A \d+ \z }xms )
            {
                ...
                if ( !$CIPIsValid ) {
                    $ValidationError{ $Type . 'IDServerError' }
                        = 'ServerError';
                }
            }
        }
        ...
        if (   $Self->{Config}->{RequestedTime}
            && $GetParam{RequestedTimeUsed} )
        {

            if (   $GetParam{RequestedTimeYear}
...
# propagate validation error to the Error variable to be detected by the frontend
            if ( $ValidationResult->{ServerError} ) {
                $ValidationError{ $DynamicFieldConfig->{Name} }
                    = ' ServerError';
            }
        }
    ...
    ATTACHMENT:
        ...
        if ( !%ValidationError ) {
            ...
            my $ChangeID = $ChangeObject->ChangeAdd(
                ...
                ITSMChangeGroupID => $GetParam{ITSMChangeGroupID},
                UserID            => $Self->{UserID},
                %AdditionalParam,
                %DynamicFieldValues,
            );
            ...
            if ($ChangeID) {
                ...
                if ( $GetParam{TicketID} ) {
                    ...
                    if ( !$LinkSuccess ) {
                        ...
                        # set error message
                        my $Message
                            = "Change with ChangeID $ChangeID was successfully added, "
                        ...
                        return $LayoutObject->ErrorScreen(
                            Message => $Message,
                            Comment =>
                                Translatable('Please contact the admin.'),
                        );
                    }
                }
                ...
                #Creating Change
                my $ConditionObject = $Kernel::OM->Get(
                    'Kernel::System::ITSMChange::ITSMCondition');
                my $WorkOrderObject = $Kernel::OM->Get(
                    'Kernel::System::ITSMChange::ITSMWorkOrder');

        #If WorkOrderID is generated then start adding actions and expressions

                my %DynamicFieldValues;
                $DynamicFieldValues{DynamicField_WorkorderTagging}
                    = $GetParam{WorkorderTagging};

                if ( $GetParam{NoofApprovers} ) {
                    ...
                    for my $i ( 1 .. $GetParam{NoofApprovers} ) {
                        ...
                        my $WorkOrderID = $WorkOrderObject->WorkOrderAdd(
                            ...
                            Instruction =>
                                "Request you to review the change and approve the workorder.",
                            WorkOrderTypeID  => 130,
                ...
                if ( !$ConditionID ) {
                    return $LayoutObject->ErrorScreen(
                        Message =>
                            Translatable('Could not add new Condition!'),
                        Comment => Translatable('Please contact the admin.'),
                    );
                }
                ...
                if ($ConditionID) {
                    ...
                    if ( !$ExpressionID ) {
                        return $LayoutObject->ErrorScreen(
                            Message =>
                                Translatable('Could not add new Expression!'),
                            Comment =>
                                Translatable('Please contact the admin.'),
                        );
                    }
                    ...
                    if ( !$ActionID ) {
                        return $LayoutObject->ErrorScreen(
                            Message =>
                                Translatable('Could not add new Action!'),
                            Comment =>
                                Translatable('Please contact the admin.'),
                        );
                    }
                ...
                for my $CachedAttachment (@CachedAttachments) {
                    ...
                    if ($Success) {
                        ...
                        if ( $CachedAttachment->{ContentID} ) {
                            ...
                            my $Search
                                = "Action=PictureUpload .+ FormID=$Self->{FormID} .+ "
                            ...
                            $ChangeData->{Description}
                                =~ s{$Search}{$Replace}xms;
                            $ChangeData->{Justification}
                                =~ s{$Search}{$Replace}xms;
                            ...
                            if ( !$Success ) {
                                $LogObject->Log(
                                    Priority => 'error',
                                    Message =>
                                        "Could not update the inline image URLs "
                        ...
                        $LogObject->Log(
                            Priority => 'error',
                            Message =>
                                'Cannot move File from Cache to VirtualFS'
    ...
    if ( $Self->{Config}->{RequestedTime} ) {
        ...
        my $TimeSelectionString = $LayoutObject->BuildDateSelection(
            ...
            RequestedTimeClass    => 'Validate '
                . ( $ValidationError{RequestedTimeInvalid} || '' ),
            Validate => 1,
            %TimePeriod,
        );
        ...
        $LayoutObject->Block(
            Name => 'RequestedTime',
            Data => { 'RequestedTimeString' => $TimeSelectionString, },
        );
    }

    my %ITSMChangeGroups = $GroupObject->PermissionUserITSMChangeGroupGet(
        UserID => $Self->{UserID},
        Type   => 'create',
    );
...
DYNAMICFIELD:
    for my $DynamicFieldConfig ( @{$DynamicField} ) {
        ...
        if ( !$Self->{Subaction} ) {
            $DynamicFieldValues{ 'DynamicField_'
        ...
        my $DynamicFieldHTML = $DynamicFieldBackendObject->EditFieldRender(
            DynamicFieldConfig => $DynamicFieldConfig,
            Value              => $DynamicFieldValues{ 'DynamicField_'
                    . $DynamicFieldConfig->{Name} },
...
ATTACHMENT:
    ...
    $Output .= $LayoutObject->Output(
        ...
        Data         => {
            %Param, %GetParam,
            %ValidationError, FormID => $Self->{FormID},
        },
    );
