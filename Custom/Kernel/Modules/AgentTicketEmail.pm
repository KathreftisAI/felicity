...
sub Run {
    ...
    for my $Key (
        ...
        NextStateID StandardTemplateID TicketSource
        )
        )
    {
...
=code debug start
	...
	open my $fh,">>","$dir/check.txt" or die "Can't open '$dir/check.txt\n'";
	print $fh "\nHai";
        print $fh $GetParam{Subject};
	print $fh "\n".$GetParam{TicketSource};
        print $fh "\nending";
	close $fh;
=cut
...
# ---
        ...
        # else set owner to auto assigned agent but do not lock it
        else {
...
# ITSMIncidentProblemManagement
# ---
#jugni linking code 20170601
#

# link tickets
#=code start

#	if( $Self->{TicketSource} ne '') {
