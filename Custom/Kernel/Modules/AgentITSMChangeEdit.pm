...
sub Run {
    ...
    for my $ParamName (
        ...
        AttachmentUpload FileID ITSMChangeGroupID
        )
        )
    {
    ...
    my $Group = $GroupObject->GroupLookup(
        GroupID => $Change->{ITSMChangeGroupID},
    );

    $ITSMChangeGroups{$Change->{ITSMChangeGroupID}}=$Group;

    $Param{ITSMChangeGroupIDSelectionString} = $LayoutObject->BuildSelection(
        Data       => \%ITSMChangeGroups,
        Name       => 'ITSMChangeGroupID',
        SelectedID => $GetParam{ITSMChangeGroupID} || $Change->{ITSMChangeGroupID},
