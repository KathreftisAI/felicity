...
sub ChangeAdd {
    ...
    return if !$Kernel::OM->Get('Kernel::System::DB')->Do(
        SQL => 'INSERT INTO change_item '
            ...
            . 'create_time, create_by, change_time, change_by, itsm_change_group_id) '
            . 'VALUES (?, ?, ?, ?, ?, ?, current_timestamp, ?, current_timestamp, ?, ?)',
        Bind => [
            \$ChangeNumber, \$ChangeStateID, \$ChangeBuilderID,
            \$CategoryID,   \$ImpactID,      \$PriorityID,
            \$Param{UserID}, \$Param{UserID}, \$Param{ITSMChangeGroupID}
        ],
    );
...
sub ChangeUpdate {
    ...
    my %Attribute = (
        ...
        ITSMChangeGroupID  => 'itsm_change_group_id',
    );
    ...
    if ($Cache) {
        ...
        return if !$Kernel::OM->Get('Kernel::System::DB')->Prepare(
            SQL => 'SELECT id, change_number, title, '
                ...
                . 'requested_time, itsm_change_group_id '
                . 'FROM change_item '
                . 'WHERE id = ? ',
            Bind  => [ \$Param{ChangeID} ],
            Limit => 1,
        ...
        while ( my @Row = $Kernel::OM->Get('Kernel::System::DB')->FetchrowArray() ) {
            ...
            $ChangeData{ITSMChangeGroupID}  = $Row[18];
        }
...
###############################################################################
####################Code Start for ITSM Change Permission added by Milan#######
###############################################################################    
    
    my $CustomPermission = $Self->CustomPermission(%Param);
...
###############################################################################
####################Code End for ITSM Change Permission added by Milan#######
############################################################################### 
...
sub CustomPermission{
    my ( $Self, %Param ) = @_;

    my $ChangeObject = $Kernel::OM->Get('Kernel::System::ITSMChange');
    my $GroupObject = $Kernel::OM->Get('Kernel::System::Group');

    if ($Param{Action} eq 'AgentITSMChangeAdd' || $Param{Action} eq 'AgentITSMChangeAddFromTemplate') {
    ...
    if ($Param{Action} eq 'AgentITSMChange' || $Param{Action} eq 'AgentITSMChangeSchedule'
        || $Param{Action} eq 'AgentITSMChangePIR' || $Param{Action} eq 'AgentITSMChangePSA'
        || $Param{Action} eq 'AgentITSMChangeMyChanges' || $Param{Action} eq 'AgentITSMChangeMyWorkOrders') {
        my %CreateGroupList = $GroupObject->PermissionUserITSMChangeGroupGet(
            UserID => $Param{UserID},
            Type   => 'ro',
        );
    ...
    if ($Param{Action} eq 'AgentITSMWorkOrderAgent') {
        ...
        if ($WorkOrderRef->{WorkOrderAgentID} && $WorkOrderRef->{WorkOrderAgentID} == $Param{UserID}) {
            return 1;
        }
    ...
    if (($Change->{ChangeManagerID} && $Change->{ChangeManagerID} == $Param{UserID}) || ($Change->{ChangeBuilderID} && $Change->{ChangeBuilderID} == $Param{UserID})) {
         return 1;
    }
    ...
    if ($GroupList{$Change->{ITSMChangeGroupID}}) {
        return 1;
    } else {
        return;
    }
