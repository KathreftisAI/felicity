...
sub CustomPermission{
    ...
    my $WorkOrderRef = $Self->WorkOrderGet(
        WorkOrderID => $Param{WorkOrderID},
        UserID      => $Param{UserID},
    );
    ...
    if ($Change->{ChangeManagerID} && ($Change->{ChangeManagerID} == $Param{UserID} || $Change->{ChangeBuilderID} == $Param{UserID})) {
        return 1;
    }


    if ($Param{Action} eq 'AgentITSMWorkOrderReport' || $Param{Action} eq 'AgentLinkObject' ) {
        if ($WorkOrderRef->{WorkOrderAgentID} && $WorkOrderRef->{WorkOrderAgentID} == $Param{UserID}) {
            return 1;
        }
    }
