...
sub Run {
    ...
    if ( $ConfigObject->Get('SessionUseCookie') ) {
        ...
        if ( $Param{SessionIDCookie} ) {
            $Param{SessionIDCookie} =~ s{[^a-zA-Z0-9_-]}{}smxg; #Sanitize to avoid tampering - whitelist allowed characters.
    ...
    if ( $Param{Action} eq 'PreLogin' ) {
        ...
        my $AESkey = $ParamObject->GetCookie(
        Key => 'Tea',
        );
 
       
        $PostPw = decrypt($PostPw, $AESkey);
        my $n = 13; # Remove Just the last 13 chars.
        $PostPw =~ s/\d{$n}$//;
