...
sub GroupAdd {
    ...
    return if !$DBObject->Do(
        SQL => 'INSERT INTO groups (name, comments, valid_id, '
            . ' create_time, create_by, change_time, change_by, itsm_change_group)'
            . ' VALUES (?, ?, ?, current_timestamp, ?, current_timestamp, ?, ?)',
        Bind => [
            \$Param{Name}, \$Param{Comment}, \$Param{ValidID}, \$Param{UserID}, \$Param{UserID}, \$Param{ITSMChange},
        ],
    );
...
sub GroupUpdate {
    ...
    my $ChangeRequired;
    KEY:
    for my $Key (qw(Name Comment ValidID ITSMChange)) {
    ...
    return if !$DBObject->Do(
        SQL => 'UPDATE groups SET name = ?, comments = ?, valid_id = ?, '
            . 'change_time = current_timestamp, change_by = ?,itsm_change_group =? WHERE id = ?',
        Bind => [
            \$Param{Name}, \$Param{Comment}, \$Param{ValidID}, \$Param{UserID}, \$Param{ITSMChange}, \$Param{ID},
        ],
    );
...
sub GroupDataList {
    ...
    return if !$DBObject->Prepare(
        SQL => 'SELECT id, name, comments, valid_id, create_time, create_by, change_time, change_by, itsm_change_group FROM groups',
    );
...
=item ITSMChangeGroupList()

returns a hash of all groups

    my %ITSMChangeGroups = $GroupObject->ITSMChangeGroupList(
        Valid => 1,   # (optional) default 0
    );
...
sub ITSMChangeGroupList {
    ...
    # get group data list
    my %ITSMChangeGroupDataList = $Self->ITSMChangeGroupDataList();

    my %GroupListValid;
    my %GroupListAll;
    KEY:
    for my $Key ( sort keys %ITSMChangeGroupDataList ) {

        next KEY if !$Key;

        # add group to the list of all groups
        $GroupListAll{$Key} = $ITSMChangeGroupDataList{$Key}->{Name};

        my $Match;
        VALIDID:
        for my $ValidID (@ValidIDs) {

            next VALIDID if $ValidID ne $ITSMChangeGroupDataList{$Key}->{ValidID};
        ...
        # add group to the list of valid groups
        $GroupListValid{$Key} = $ITSMChangeGroupDataList{$Key}->{Name};
    }
...
=item ITSMChangeGroupDataList()

returns a hash of all group data

    my %ITSMChangeGroupDataList = $GroupObject->GroupDataList();

the result looks like

    %ITSMChangeGroupDataList = (
        1 => {
            ID         => 1,
            Name       => 'Group 1',
            Comment    => 'The Comment of Group 1',
...
sub ITSMChangeGroupDataList {
    ...
    # get all group data from database
    return if !$DBObject->Prepare(
        SQL => 'SELECT id, name, comments, valid_id, create_time, create_by, change_time, change_by, itsm_change_group FROM groups where itsm_change_group = 1',
    );

    # fetch the result
    my %ITSMChangeGroupDataList;
    while ( my @Row = $DBObject->FetchrowArray() ) {

        $ITSMChangeGroupDataList{ $Row[0] } = {
            ID         => $Row[0],
            Name       => $Row[1],
            Comment    => $Row[2] || '',
            ValidID    => $Row[3],
    ...
    return %ITSMChangeGroupDataList;
}



=item PermissionUserITSMGroupGet()

returns a list of groups a user is member of

    my %ITSMGroupList = $GroupObject->PermissionUserGroupGet(
        UserID => 123,
...
sub PermissionUserITSMChangeGroupGet {
    my ( $Self, %Param ) = @_;
    ...
    my %ITSMChangeGroups = $Self->ITSMChangeGroupList(
        Valid => 1, 
    );

    my %ITSMUserGroups;
    foreach my $keys (keys %Groups) {
        if (exists $ITSMChangeGroups{$keys}){
            $ITSMUserGroups{$keys}= $ITSMChangeGroups{$keys};
        }
    }

    return %ITSMUserGroups;
}
