...
sub _TicketGetFirstResponse {
    ...
    return if !$DBObject->Prepare(
        SQL => 'SELECT a.create_time,a.id FROM article a, article_sender_type ast, article_type art'
            ...
            . ' a.ticket_id = ?  AND'
            . ' (art.name LIKE \'email-ext%\' OR art.name LIKE \'email-notif%\' OR art.name LIKE \'note-ext%\'  OR art.name = \'fax\' OR art.name = \'sms\')'
            . ' AND a.a_subject NOT LIKE \'%Create%\' AND a.a_subject NOT LIKE \'%SLA%\' ORDER BY a.create_time',
...
# Start of Code by Jagruti Sangani -JS 
	my $start_time = $Ticket{Created};
	my $DBObject = $Kernel::OM->Get('Kernel::System::DB');


# 22 means Priority Update && 27 means StateUpdate
	return if !$DBObject->Prepare(
			SQL  => 'SELECT count(*) FROM ticket_history WHERE history_type_id = 22 AND ticket_id = ?',
			Bind => [ \$Param{TicketID} ],
	);

	my $count_severity_change_total = 0;	
	my $severity_update_time = "";
	while ( my @Row = $DBObject->FetchrowArray() ) 
	{
	 	$count_severity_change_total = $Row[0];
	}

	my $query_transition_history = 'SELECT th.name,tsh.name, tsh.id,unix_timestamp(th.change_time) from ticket_history as th,ticket_state as ts,ticket_state_type as tsh WHERE th.ticket_id = ? AND th.history_type_id = 27 AND th.state_id = ts.id AND ts.type_id = tsh.id';
	my $latest_severity_id = 0;
		 
		 
	#Check if there are any changes in severity that were made during the life cycle of the ticket
	if($count_severity_change_total > 0)
		{
			#Get the History id of the last severity
			#Fire Query
			return if !$DBObject->Prepare(
				SQL  => 'SELECT id,change_time FROM ticket_history WHERE history_type_id = 22 AND ticket_id = ? ORDER BY change_time DESC LIMIT 1 OFFSET 0',
				Bind => [ \$Param{TicketID} ],
			);
			#Get Result
			...
			while ( my @Row = $DBObject->FetchrowArray() ) 
			{
			 	$latest_severity_id = $Row[0];
				$severity_update_time = $Row[1];
			}
			$query_transition_history .= ' AND th.id >= ' . $latest_severity_id;

		    # Commented by Akshay Suresh ~AS $start_time = $severity_update_time;
		}
	$query_transition_history .= ' ORDER BY th.change_time';
		 
	#Query is prepared. Now the query is fired.
	# BLock of code added by Anil G ~AG
	my $ticket_history_id = 0;
	my $ticket_history_name = "";
	my $ticket_history_timestamp_epoch = 0;
	my $is_inside_pending = 'false';
	my $total_time_on_hold = 0;
	my $temp_pending_start_time = 0;
	my $ticket_history_status_id = 0;
	my $temp_pending_ending_time = 0;
	my $created_in_pending = 0;
	my $created_in_pending_date = 0;
	my $query_created_in_pending = '';
		
	if($count_severity_change_total > 0)
	{
		$query_created_in_pending = 'SELECT tst.id,(SELECT unix_timestamp(th.create_time) from ticket_history as th WHERE th.ticket_id = ? ORDER by th.create_time LIMIT 1) as create_time from ticket_state_type as tst,ticket_state as ts WHERE ts.type_id = tst.id AND ts.id = (SELECT th.state_id from ticket_history as th WHERE th.ticket_id = ? AND th.id < ' . $latest_severity_id . ' ORDER by th.create_time DESC LIMIT 1)';

		return if !$DBObject->Prepare(
                SQL  => $query_created_in_pending,
                Bind => [ \$Param{TicketID},\$Param{TicketID}],  # You'll need to use the ticket id somewhere as parameter
     	);
		
		while ( my @Row = $DBObject->FetchrowArray() ) 
		{
			 $created_in_pending = $Row[0];
			 $created_in_pending_date = $Row[1];

			# 5 means pending auto
			if($created_in_pending == 5)
			{ 
				# Conditional to check if the ticket was created in a pending state
				 $is_inside_pending = 'true';
				 $temp_pending_start_time =  $created_in_pending_date;	
			}
		}
	}
#End of block of code		 
	...
	return if !$DBObject->Prepare(
		SQL  => $query_transition_history,
		Bind => [ \$Param{TicketID} ],
	);
		 
	while ( my @Row = $DBObject->FetchrowArray() ) 
	{
		#Here the pseudo-code will be setup
		$ticket_history_name = $Row[0];
		$ticket_history_status_id = $Row[2];
		$ticket_history_timestamp_epoch = $Row[3];
		
		if($is_inside_pending eq 'false')
			{
				if($ticket_history_status_id == 5)
					{
					$is_inside_pending = 'true';
					#Set temp_pending_start_time in epoch
					$temp_pending_start_time = $ticket_history_timestamp_epoch;
					}
				else
					{
			...
			{
			if($ticket_history_status_id == 2)
				{
				#Set temp_pending_ending_time in epoch
				$temp_pending_ending_time = $ticket_history_timestamp_epoch;

				# Add difference of temp_pending_start_time and temp_pending_ending_time to total_time_on_hold

				$total_time_on_hold += ($temp_pending_ending_time - $temp_pending_start_time);

				$is_inside_pending = 'false';
				$temp_pending_start_time = 0;
				$temp_pending_ending_time = 0;
				}
			else
				{
	...
	#The ticket is still in a pending state
	if($is_inside_pending eq 'true')
		{
			#Get the current time in epoch
			my $currentEpoch = time();
			
			#my $offset_Constant = 0;
			my $offset_Constant = 19800;		

			#Add the difference to the resultant hold time
			my $difference = ($currentEpoch - $temp_pending_start_time + $offset_Constant);

			$total_time_on_hold += $difference;
		}
    ...
    if ( $Ticket{StateType} && $Ticket{StateType} =~ /^(merge|close|remove)/i ) {
        ...
        my %EscalationTimes = (
            ...
            EscalationUpdateTime   => 'escalation_update_
time',
    ...
    if ( !$Escalation{FirstResponseTime} ) {
        ...
        # update first response time to expected escalation destination time
            ...
            # get time object
            my $TimeObject = $Kernel::OM->Get('Kernel::System::Time');
#Jugni added + $total_time_on_hold in Time
            my $DestinationTime = $TimeObject->DestinationTime(
                StartTime => $TimeObject->TimeStamp2SystemTime(
                    String => $Ticket{Created}
                ),
                Time     => ($Escalation{FirstResponseTime} * 60 + $total_time_on_hold),
                Calendar => $Escalation{Calendar},
            );
...
#Jugni added + $total_time_on_hold in Time
            my $DestinationTime = $TimeObject->DestinationTime(
                ...
                Time     => ($Escalation{UpdateTime} * 60 + $total_time_on_hold),
                Calendar => $Escalation{Calendar},
            );
...
#Jugni added + $total_time_on_hold in Time
            my $DestinationTime = $TimeObject->DestinationTime(
                ...
                Time     => ($Escalation{SolutionTime} * 60  + $total_time_on_hold),
                Calendar => $Escalation{Calendar},
            );
