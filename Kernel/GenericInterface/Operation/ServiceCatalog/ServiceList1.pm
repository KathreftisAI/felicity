...
package Kernel::GenericInterface::Operation::ServiceCatalog::ServiceList1;
...
sub Run {
    ...
    my $serviceListObject
        = $Self->{ServiceObject}->ServiceListGet( UserID => $UserID );
    push @Item, $serviceListObject;

    my $DBObject = $Kernel::OM->Get('Kernel::System::DB');
    my $SQL      = "SELECT service_id,sla_id FROM service_sla";
    return if !$DBObject->Prepare( SQL => $SQL );
    while ( my @Row = $DBObject->FetchrowArray() ) {

        #      print $serviceListObject;
        #      print "\n";
    }
