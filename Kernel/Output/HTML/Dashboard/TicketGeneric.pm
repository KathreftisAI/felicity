...
sub Run {
    ...
    for my $TicketID ( @{$TicketIDs} ) {
        ...
        my %Ticket = $TicketObject->TicketGet(
            TicketID      => $TicketID,
            UserID        => $Self->{UserID},
            DynamicFields => 1,			#DynamicField change to 1 for displaying severity by ashish
            Silent        => 1
        );
        ...
        for my $Column (@Columns) {
		   ...
		   #code added by ashish to display colour in EscalationResponseTime & EscalationSolutionTime on the basis of Severity              
		   if($Column eq 'EscalationResponseTime' || $Column eq 'EscalationSolutionTime'){
                       if ($DataValue =~ /[-]/){
                            $CSSClass="EscalationS1";
                        }
                        else{
                            $CSSClass="Escalation$Ticket{DynamicField_Severity}";  
                       }
                   }
		#code ended to display colour 
		   $LayoutObject->Block(
