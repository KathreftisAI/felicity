...
sub new {
    ...
    $Self->{DynamicField}
        ...
        FieldFilter => $ConfigObject->Get("Ticket::Frontend::$Self->{Action}")
            ->{DynamicField} || {},
        );

    # get form id
    $Self->{FormID} = $Kernel::OM->Get('Kernel::System::Web::Request')
        ->GetParam( Param => 'FormID' );

    # create form id
    if ( !$Self->{FormID} ) {
        $Self->{FormID} = $Kernel::OM->Get('Kernel::System::Web::UploadCache')
            ->FormIDCreate();
    }
...
sub Run {
    ...
    #jugni custom code. Added TicketSource in below key array
    for my $Key (
        ...
        StandardTemplateID FromChatID TicketSource DestQueue DestCategory DestSubCategory
        )
        )
    {
    ...
    my $IsUpload
        = ( $ParamObject->GetParam( Param => 'AttachmentUpload' ) ? 1 : 0 );

    # MultipleCustomer From-field
    my @MultipleCustomer;
    my $CustomersNumber
        = $ParamObject->GetParam(
        Param => 'CustomerTicketCounterFromCustomer' )
        || 0;
    my $Selected
        = $ParamObject->GetParam( Param => 'CustomerSelected' ) || '';
    ...
    if ($CustomersNumber) {
        ...
        for my $Count ( 1 ... $CustomersNumber ) {
            my $CustomerElement = $ParamObject->GetParam(
                Param => 'CustomerTicketText_' . $Count );
            my $CustomerSelected
                = ( $Selected eq $Count ? 'checked="checked"' : '' );
            my $CustomerKey
                = $ParamObject->GetParam( Param => 'CustomerKey_' . $Count )
                || '';
            ...
            if ($CustomerElement) {
                ...
                if ( !$IsUpload ) {
                    ...
                    if ( defined $AddressesList{$CustomerElement}
                        && $CustomerError eq '' )
                    {
...
DYNAMICFIELD:
    ...
    $GetParam{DynamicField_ITSMImpact}
        = $ParamObject->GetParam( Param => 'DynamicField_ITSMImpact' );
    ...
    if ( $GetParam{ServiceID} ) {
        ...
        if (  !$GetParam{DynamicField_ITSMImpact}
            && $GetParam{ElementChanged} ne 'DynamicField_ITSMImpact' )
        {
        ...
        if ( $GetParam{PriorityRC} && $GetParam{DynamicField_ITSMImpact} ) {
            ...
            # get priority
            $GetParam{PriorityIDFromImpact}
                = $Kernel::OM->Get('Kernel::System::ITSMCIPAllocate')
    ...
    if (   defined $GetParam{Year}
        && defined $GetParam{Month}
        && defined $GetParam{Day}
        && defined $GetParam{Hour}
        && defined $GetParam{Minute} )
    {
        %GetParam = $LayoutObject->TransformDateSelection( %GetParam, );
    }
    ...
    if ( $GetParam{FromChatID} ) {
        ...
        # Ok, take the chat
        my %ChatParticipant
            = $Kernel::OM->Get('Kernel::System::Chat')->ChatParticipantCheck(
        ...
        if (   $PermissionLevel ne 'Owner'
            && $PermissionLevel ne 'Participant' )
        {
            return $LayoutObject->FatalError(
                Message => Translatable('No permission.'),
    ...
    if ( !$Self->{Subaction} || $Self->{Subaction} eq 'Created' ) {
        ...
        if (   $Self->{LastScreenOverview}
            ...
            && $Self->{RequestedURL}
            !~ /Action=AgentTicketPhone.*LinkTicketID=/ )
        {
   ...
   # check if article is from the same TicketID as we checked permissions for.
            ...
            if ( IsStringWithData( $Article{To} ) ) {
                ...
                if ( $ConfigObject->{CustomerPanelOwnSelection} ) {
                    for my $Queue (
                        sort
                        keys %{ $ConfigObject->{CustomerPanelOwnSelection} }
                        )
                    {
                        my $Value = $ConfigObject->{CustomerPanelOwnSelection}
                            ->{$Queue};
                        $Queues{$Queue} = $Value;
                    }
                }
                ...
                my %SystemAddressLookup
                    = reverse $Kernel::OM->Get(
                    'Kernel::System::SystemAddress')->SystemAddressList();
                ...
                if (  !defined $QueueLookup{ $Article{To} }
                    && defined $SystemAddressLookup{$SystemAddressEmail} )
                {
...
# ACLs info for all fields
    ...
    DYNAMICFIELD:
        for my $DynamicFieldConfig ( @{ $Self->{DynamicField} } ) {
            ...
            if ($UserPreferences{ 'UserDynamicField_'
                        . $DynamicFieldConfig->{Name} } )
            {
                $DefaultValue = $UserPreferences{ 'UserDynamicField_'
                        . $DynamicFieldConfig->{Name} };
            }
            ...
            next DYNAMICFIELD
                ...
                && !IsArrayRefWithData($DefaultValue);

            $DynamicFieldDefaults{ 'DynamicField_'
                    . $DynamicFieldConfig->{Name} } = $DefaultValue;
        }
...
# this information will be available in the SplitTicketParam hash
        if ( $SplitTicketParam{TicketID} ) {
            ...
            for my $SplitedParam (qw(TypeID ServiceID SLAID PriorityID)) {
                $SplitTicketParam{$SplitedParam}
                    = $SplitTicketData{$SplitedParam};
            }
            ...
            $SplitTicketParam{ResponsibleUserSelected}
                = $SplitTicketData{ResponsibleID};
            ...
            for my $Key ( sort keys %SplitTicketData ) {
                if ( $Key =~ /DynamicField\_(.*)/ ) {
                    $SplitTicketParam{DynamicField}{$1}
                        = $SplitTicketData{$Key};
...
# the dynamic field, otherwise will use Dynamic Field default value
                ...
                if ($UserPreferences{ 'UserDynamicField_'
                            . $DynamicFieldConfig->{Name} } )
                {
                    $Value = $UserPreferences{ 'UserDynamicField_'
                            . $DynamicFieldConfig->{Name} };
                }
            }
        ...
        if ( !$Subject ) {
            $Subject
                = $LayoutObject->Output( Template => $Config->{Subject} || '',
                );
        }
        my $Body = $Article{Body} || '';
        if ( !$Body ) {
            $Body
                = $LayoutObject->Output( Template => $Config->{Body} || '', );
        }

        # make sure body is rich text (if body is based on config)
        if ( !$GetParam{ArticleID} && $LayoutObject->{BrowserRichText} ) {
            $Body = $LayoutObject->Ascii2RichText( String => $Body, );
        }
...
# in order to set correct ACLs on page load (initial). See bug 8687.
        if (   IsHashRefWithData( \%SplitTicketParam )
        ...
        $Output .= $Self->_MaskPhoneNew(
            ...
            StandardTemplates => $Self->_GetStandardTemplates(
                %GetParam,         %ACLCompatGetParam,
                %SplitTicketParam, QueueID => $Self->{QueueID} || '',
            ),
    ...
    # create new ticket and article
        ...
        if ( $GetParam{NextStateID} ) {
            %StateData = $Kernel::OM->Get('Kernel::System::State')
                ->StateGet( ID => $GetParam{NextStateID}, );
        }
        ...
        if ( $Dest && $Dest !~ m{ \A (\d+)? \| \| .+ \z }xms ) {
            ...
            my $DestID = $QueueObject->QueueLookup( Queue => $Dest, );
        ...
        my $SelectedCustomerUser
            ...
            || '';
        my $CustomerID
            = $ParamObject->GetParam( Param => 'CustomerID' ) || '';
        my $ExpandCustomerName
            = $ParamObject->GetParam( Param => 'ExpandCustomerName' )
            || 0;
        ...
        for my $Count ( 1 .. 2 ) {
            my $Item
                = $ParamObject->GetParam(
                Param => "ExpandCustomerName$Count" )
                || 0;
            if ( $Count == 1 && $Item ) {
        ...
        if ( $GetParam{Body} && !$LayoutObject->{BrowserRichText} ) {
            $GetParam{Body} = $LayoutObject->WrapPlainText(
                MaxCharacters =>
                    $ConfigObject->Get('Ticket::Frontend::TextAreaNote'),
    ...
    COUNT:
        ...
        if ( $ParamObject->GetParam( Param => 'AttachmentUpload' ) ) {
            ...
            my %UploadStuff
                = $ParamObject->GetUploadAll( Param => 'FileUpload', );
            ...
            if ( !$IsUpload && !$ExpandCustomerName ) {
                ...
                if ( !IsHashRefWithData($ValidationResult) ) {
                    return $LayoutObject->ErrorScreen(
                        ...
                        Comment =>
                            Translatable('Please contact the administrator.'),
                    );
                }
...
# propagate validation error to the Error variable to be detected by the frontend
        ...
        if ( $ExpandCustomerName == 1 ) {
            ...
            for my $KeyCustomerUser ( sort keys %CustomerUserList ) {
                ...
                $Param{CustomerUserListLast}
                    = $CustomerUserList{$KeyCustomerUser};
            ...
            if ( $Param{CustomerUserListCount} == 1 ) {
                ...
                if ( $FromExternalCustomer{Customer} ) {
                    my %ExternalCustomerUserData
                        = $CustomerUserObject->CustomerUserDataGet(
                        User => $FromExternalCustomer{Customer}, );
                    $FromExternalCustomer{Email}
                        = $ExternalCustomerUserData{UserEmail};
                }
            }
        ...
        # reset previous ServiceID to reset SLA-List if no service is selected
            ...
            $Output .= $Self->_MaskPhoneNew(
                ...
                CustomerID =>
                    $LayoutObject->Ascii2Html( Text => $CustomerID ),
                CustomerUser => $CustomerUser,
                CustomerData => \%CustomerData,
                To           => $Self->_GetTos(
                    %GetParam, %ACLCompatGetParam, QueueID => $NewQueueID
                ),
                ToSelected  => $Dest,
                Errors      => \%Error,
                Attachments => \@Attachments,
    ...
    DYNAMICFIELD:
        ...
        if ( $GetParam{ServiceID} && $Service{Criticality} ) {
            ...
            my $CriticalityDynamicFieldConfig
                = $Kernel::OM->Get('Kernel::System::DynamicField')
                ->DynamicFieldGet( Name => 'ITSMCriticality', );

            # get possible values for criticality
            my $CriticalityPossibleValues
                = $DynamicFieldBackendObject->PossibleValuesGet(
                DynamicFieldConfig => $CriticalityDynamicFieldConfig, );

            # reverse the list to find out the key
            my %ReverseCriticalityPossibleValues
                = reverse %{$CriticalityPossibleValues};
            ...
            $DynamicFieldBackendObject->ValueSet(
                ...
                ObjectID           => $TicketID,
                Value =>
                    $ReverseCriticalityPossibleValues{ $Service{Criticality}
                    },
                UserID => $Self->{UserID},
            );
        }
        ...
        if ( $LayoutObject->{BrowserRichText} ) {
            $PlainBody
                = $LayoutObject->RichText2Ascii( String => $GetParam{Body} );
        }
        ...
        my $ArticleID = $TicketObject->ArticleCreate(
            ...
            AutoResponseType =>
                ( $ConfigObject->Get('AutoResponseForWebTickets') )
            ? 'auto reply'
            : '',
            OrigHeader => {
                From    => $GetParam{From},
    ...
    DYNAMICFIELD:
        for my $DynamicFieldConfig ( @{ $Self->{DynamicField} } ) {
            next DYNAMICFIELD if !IsHashRefWithData($DynamicFieldConfig);
            next DYNAMICFIELD
                if $DynamicFieldConfig->{ObjectType} ne 'Article';
        ...
        if ( $GetParam{FromChatID} ) {
            ...
            my %Chat
                = $ChatObject->ChatGet( ChatID => $GetParam{FromChatID}, );
            ...
            if (@ChatMessageList) {
                ...
                my $JSONBody = $Kernel::OM->Get('Kernel::System::JSON')
                    ->Encode( Data => \@ChatMessageList, );
                ...
                $ChatArticleID = $TicketObject->ArticleCreate(
                    ...
                    From          => $GetParam{From},
                    To            => $GetParam{To},
                    Subject       => $Kernel::OM->Get('Kernel::Language')
                        ->Translate('Chat'),
                    Body           => $JSONBody,
                    ...
                    HistoryComment => $Config->{HistoryComment} || '%%',
                    Queue =>
                        $QueueObject->QueueLookup( QueueID => $NewQueueID ),
                );
            }
            if ($ChatArticleID) {
                ...
                # otherwise set chat status to closed and inform other agents
                    ...
                    my $LeaveMessage
                        = $Kernel::OM->Get('Kernel::Language')
                        ->Translate( "%s has left the chat.",
                        $RequesterName, );
                    ...
                    my $ChatTTL = $Kernel::OM->Get('Kernel::Config')
                        ->Get('ChatEngine::ChatTTL');

                    my $ChatClosedMessage
                ...
                CHATPARTICIPANT:
                    for my $ChatParticipant (@ParticipantsList) {
                        ...
                        next CHATPARTICIPANT
                            if $ChatParticipant->{ChatterType} ne 'User';
        ...
        # ITSMIncidentProblemManagement
        # ---
        # jugni custom code for linking two tickets

        $Kernel::OM->Get('Kernel::System::LinkObject')->LinkAdd(
            SourceObject => 'Ticket',
            SourceKey    => $GetParam{TicketSource},
            TargetObject => 'Ticket',
            TargetKey    => $TicketID,
            Type         => 'Normal',
            State        => 'Valid',
        ...
        my $TempLinkList
            = $Kernel::OM->Get('Kernel::System::LinkObject')->LinkList(
            Object => 'Ticket',
            Key    => $Self->{FormID},
            State  => 'Temporary',
            UserID => $Self->{UserID},
            ...
            for my $TargetObjectOrg ( sort keys %{$TempLinkList} ) {
                ...
                for my $Type ( sort keys %{$TypeList} ) {
                    ...
                    for my $Direction ( sort keys %{$DirectionList} ) {
                        ...
                        for my $TargetKeyOrg (
                            sort keys %{ $DirectionList->{$Direction} } )
                        {
                            ...
                            my $Success
                                = $Kernel::OM->Get(
                                'Kernel::System::LinkObject')->LinkAdd(
    ...
    # ---
        ...
        if ( $ServiceID && $Config->{ShowIncidentState} ) {
            ...
            %Response = (
                ...
                CurInciState  => $LayoutObject->{LanguageObject}
                    ->Translate( $Service{CurInciState} ),
            );
        }

        # encode response to JSON
        my $JSON = $LayoutObject->JSONEncode( Data => \%Response, );
    ...
    # ---
        ...
        my $StandardTemplates
            = $Self->_GetStandardTemplates( %GetParam, %ACLCompatGetParam,
            QueueID => $QueueID || '', );
...
# convert possible values key => value to key => key for ACLs using a Hash slice
     ...
     # get the template text and set new attachments if a template is selected
            if ( IsPositiveInteger( $GetParam{StandardTemplateID} ) ) {
                my $TemplateGenerator
                    = $Kernel::OM->Get('Kernel::System::TemplateGenerator');
    ...
    # set template text, replace smart tags (limited as ticket is not created)
                ...
                my $StdAttachmentObject
                    = $Kernel::OM->Get('Kernel::System::StdAttachment');
                ...
                for ( sort keys %AllStdAttachments ) {
                    my %AttachmentsData
                        = $StdAttachmentObject->StdAttachmentGet( ID => $_ );
                    $UploadCacheObject->FormIDAddFile(
                        FormID      => $Self->{FormID},
                        Disposition => 'attachment',
                        %AttachmentsData,
...
sub _GetNextStates {
    ...
    if ( $Param{QueueID} || $Param{TicketID} ) {
        %NextStates
            = $Kernel::OM->Get('Kernel::System::Ticket')->TicketStateList(
            %Param,
            Action => $Self->{Action},
            UserID => $Self->{UserID},
            );
...
sub _GetUsers {
    ...
    my %ShownUsers;
    my %AllGroupsMembers
        = $Kernel::OM->Get('Kernel::System::User')->UserList(
        Type  => 'Long',
        Valid => 1,
        );
    ...
    if ( $Kernel::OM->Get('Kernel::Config')
        ->Get('Ticket::ChangeOwnerToEveryone') )
    {
...
sub _GetPriorities {
    ...
    if ( $Param{QueueID} || $Param{TicketID} ) {
        %Priorities
            = $Kernel::OM->Get('Kernel::System::Ticket')->TicketPriorityList(
            %Param,
            Action => $Self->{Action},
            UserID => $Self->{UserID},
            );
...
sub _GetServices {
    ...
    my $DefaultServiceUnknownCustomer
        ...
        ->Get('Ticket::Service::Default::UnknownCustomer');
...
# if $DefaultServiceUnknownCustomer = 1 set CustomerUserID to get default services
    ...
    if ( $Param{CustomerUserID} ) {
        %Service
            = $Kernel::OM->Get('Kernel::System::Ticket')->TicketServiceList(
            %Param,
            Action => $Self->{Action},
            UserID => $Self->{UserID},
            );
...
sub _GetTos {
    ...
    if ( $ConfigObject->Get('Ticket::Frontend::NewQueueOwnSelection') ) {
        %NewTos
            = %{ $ConfigObject->Get('Ticket::Frontend::NewQueueOwnSelection')
            };
    }
    else {
        ...
        my %Tos;
        if ( $ConfigObject->Get('Ticket::Frontend::NewQueueSelectionType') eq
            'Queue' )
        {
        ...
        my %UserGroups
            = $Kernel::OM->Get('Kernel::System::Group')->PermissionUserGet(
            UserID => $Self->{UserID},
            Type   => 'create',
            );
    ...
    QUEUEID:
        for my $QueueID ( sort keys %Tos ) {
            my %QueueData = $Kernel::OM->Get('Kernel::System::Queue')
                ->QueueGet( ID => $QueueID );
            ...
            if ( $ConfigObject->Get('Ticket::Frontend::NewQueueSelectionType')
                ne 'Queue' )
            {
                my %SystemAddressData
                    = $Kernel::OM->Get('Kernel::System::SystemAddress')
                    ->SystemAddressGet( ID => $Tos{$QueueID}, );
...
sub _MaskPhoneNew {
    ...
    $LayoutObject->Block( Name => 'CustomerSearchAutoComplete', );
    ...
    if ( $Param{MultipleCustomer} ) {
        for my $Item ( @{ $Param{MultipleCustomer} } ) {
            ...
            if ( $Item->{CustomerError} ) {
                $LayoutObject->Block( Name => 'CustomerErrorExplantion', );
            }
            $CustomerCounter++;
        }
    }
    ...
    $LayoutObject->Block(
        ...
        Data => { CustomerCounter => $CustomerCounter++, },
    );

    if (   $Param{FromInvalid}
        && $Param{Errors}
        && !$Param{Errors}->{FromErrorType} )
    {
...
ATTACHMENT:
    ...
    if ( $Param{FromChatID} ) {
        my @ChatMessages = $Kernel::OM->Get('Kernel::System::Chat')
            ->ChatMessageList( ChatID => $Param{FromChatID}, );
