...
package Kernel::Modules::CMDBComputers;
...
sub Run {
    ...
    my $query =
"Select id,session_id,token FROM sso_servetrs_tokens WHERE session_id LIKE ?";
    $DBObject->Prepare(
        SQL   => $query,
        Bind  => [ \$Self->{SessionID} ],
        Limit => 1,
...
"Select id,server_ip FROM sso_servers WHERE server_name LIKE 'glpi'";
    ...
    $mydata{'url'} =
      $glpiurl . "computer.php?embed=felicity" . "&token=" . $Token;
    $mydata{'title'} = "CMDB All Computers";
