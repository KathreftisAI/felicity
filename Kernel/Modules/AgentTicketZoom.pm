...
sub new {
    ...
    $Self->{ZoomExpandSort}
        = $ParamObject->GetParam( Param => 'ZoomExpandSort' );
    ...
    if ( !$Self->{Subaction} ) {
        ...
        {
            $Self->{ZoomExpand}
                = $ConfigObject->Get('Ticket::Frontend::ZoomExpand');
            if ( $UserPreferences{UserLastUsedZoomViewType} ) {
                if ( $UserPreferences{UserLastUsedZoomViewType} eq 'Expand' )
                {
                    ...
                    $UserPreferences{UserLastUsedZoomViewType} eq 'Collapse' )
                {
                    $Self->{ZoomExpand} = 0;
                }
                elsif (
        ...
        elsif ( defined $Self->{ZoomExpand} || defined $Self->{ZoomTimeline} )
        {
            ...
            elsif ( defined $Self->{ZoomTimeline}
                && $Self->{ZoomTimeline} == 1 )
            {
    ...
    if ( !defined $Self->{ZoomExpandSort} ) {
        $Self->{ZoomExpandSort}
            = $ConfigObject->Get('Ticket::Frontend::ZoomExpandSort');
    }

    $Self->{ArticleFilterActive}
        = $ConfigObject->Get('Ticket::Frontend::TicketArticleFilter');
    ...
    # ticket id lookup
    if (  !$Self->{TicketID}
        && $ParamObject->GetParam( Param => 'TicketNumber' ) )
    {
    ...
    $Self->{HistoryTypeMapping} = {
        ...
        EscalationUpdateTimeStart =>
            Translatable('Escalation Update Time In Effect'),
        EscalationUpdateTimeStop =>
            Translatable('Escalation Update Time Stopped'),
        EscalationFirstResponseTimeStop =>
            Translatable('Escalation First Response Time Stopped'),
        ...
        EscalationSolutionTimeStart =>
            Translatable('Escalation Solution Time In Effect'),
        EscalationSolutionTimeStop =>
            Translatable('Escalation Solution Time Stopped'),
        EscalationResponseTimeStart =>
            Translatable('Escalation Response Time In Effect'),
        EscalationResponseTimeStop =>
            Translatable('Escalation Response Time Stopped'),
    ...
    if ( defined $ConfigObject->Get('TimelineViewEnabled')
        && $ConfigObject->Get('TimelineViewEnabled') == 1 )
    {
        my $ZoomFrontendConfiguration
            = $ConfigObject->Get('Frontend::Module')->{AgentTicketZoom};
        my @CustomJSFiles = ( 'Core.Agent.TicketZoom.TimelineView.js', );
        push(
            @{ $ZoomFrontendConfiguration->{Loader}->{JavaScript} || [] },
            @CustomJSFiles
        );
    }
...
sub Run {
    ...
    if ( $Self->{Subaction} eq 'TicketMarkAsSeen' ) {
        ...
        if ( $Ticket{ArchiveFlag} ne 'y' ) {
            $Success
                = $Self->_TicketItemSeen( TicketID => $Self->{TicketID} );
        }
...
# Always use user id 1 because other users also have to see the important flag
        ...
        return $LayoutObject->Redirect( OP =>
                "Action=AgentTicketZoom;TicketID=$Self->{TicketID};ArticleID=$Self->{ArticleID}",
        );
    }
    ...
    if ( $Self->{Subaction} eq 'MarkAsSeen' ) {
        ...
        if ( $Ticket{ArchiveFlag} ne 'y' ) {
            $Success
                = $Self->_ArticleItemSeen( ArticleID => $Self->{ArticleID} );
        }
    ...
    # article update
        ...
        my %StandardTemplates
            = $Kernel::OM->Get('Kernel::System::Queue')
    ...
    if ( $Self->{Subaction} eq 'ArticleFilterSet' ) {
        ...
        my @ArticleTypeFilterIDs
            = $ParamObject->GetArray( Param => 'ArticleTypeFilter' );
        ...
        if ($Update) {
            $JSON = $LayoutObject->JSONEncode(
                Data => {
                    Message =>
                        Translatable('Article filter settings were saved.'),
                },
            );
        }
    ...
    if ( $Self->{Subaction} eq 'EvenTypeFilterSet' ) {
        ...
        my $TicketID     = $ParamObject->GetParam( Param => 'TicketID' );
        my $SaveDefaults = $ParamObject->GetParam( Param => 'SaveDefaults' );
        my @EventTypeFilterIDs
            = $ParamObject->GetArray( Param => 'EventTypeFilter' );
    ...
    if ( $Self->{ArticleFilterActive} ) {
        ...
        my $ArticleFilterSessionString
            = $Self->{ 'ArticleFilter' . $Self->{TicketID} };
     ...
     # do not use defaults for this ticket if filter was explicitly turned off
        ...
        my @tempArticleTypeIDs = $Self->{ArticleFilter}->{ArticleTypeID};
        ...
        return
            if !$DBObject->Prepare(
            SQL =>
                'SELECT id from article_type WHERE name NOT LIKE \'%email-notif%\'',
            Limit => 50,
            );
        ...
        while ( my @Row = $DBObject->FetchrowArray() ) {
            ...
            push @filters123, $Data{FirstResponse};
        }

        push @tempArticleTypeIDs, @filters123;
        $Self->{ArticleFilter}->{ArticleTypeID} = \@tempArticleTypeIDs;

        # extract ArticleSenderTypeIDs
        if (   $ArticleFilterSessionString
            && $ArticleFilterSessionString
            =~ m{ ArticleSenderTypeFilter < ( [^<>]+ ) > }xms )
        {
            my @IDs = split /,/, $1;
        ...
        my $EventTypeFilterSessionString
            = $Self->{ 'EventTypeFilter' . $Self->{TicketID} };
    ...
    if ( $Self->{Subaction} eq 'ShowHTMLeMail' ) {
        ...
        if ( !$Self->{ArticleID} ) {
            return $LayoutObject->ErrorScreen(
                Message => Translatable('Need ArticleID!') );
        }
        ...
        if ( !%Article ) {
            return $LayoutObject->ErrorScreen(
                Message => Translatable('Invalid ArticleID!') );
        }
...
sub MaskAgentZoom {
    ...
    # owner info
    my %OwnerInfo = $UserObject->GetUserData( UserID => $Ticket{OwnerID}, );

    # responsible info
    my %ResponsibleInfo
        = $UserObject->GetUserData( UserID => $Ticket{ResponsibleID} || 1, );
    ...
    my @ArticleBoxAll
        = $TicketObject->ArticleContentIndex(@ArticleContentArgsAll);
    ...
    my @ArticleContentArgs = (
        ...
        DynamicFields => 0,   # fetch later only for the article(s) to display
        %{ $Self->{ArticleFilter} // {} }
        ,                     # limit by ArticleSenderTypeID/ArticleTypeID

    );

    # get content
    my @ArticleBox = $TicketObject->ArticleContentIndex( @ArticleContentArgs,
        Page => $Page, );
    ...
    if ( !@ArticleBox && $Page > 1 ) {
        ...
        $Page       = 1;
        @ArticleBox = $TicketObject->ArticleContentIndex( @ArticleContentArgs,
            Page => $Page, );
    ...
    if ( scalar @ArticleBox != scalar @ArticleBoxAll ) {
        ...
        if ( $ConfigObject->Get('Ticket::Frontend::ZoomExpandSort') eq
            'reverse' )
        {
...
ARTICLE:
    for my $Article (@ArticleBox) {
        ...
        if ( scalar @ArticleBox != scalar @ArticleBoxAll ) {
            my @ArticleOnPage
                = grep { $_->{ArticleID} =~ $Article->{ArticleID} }
                @ArticleBoxAll;
    ...
    my $ArticleMaxLimit
        = $ConfigObject->Get('Ticket::Frontend::MaxArticlesZoomExpand')
        // 400;
    ...
    if ($IsProcessTicket) {
        $Param{WidgetTitle}
            = $Self->{DisplaySettings}->{ProcessDisplay}->{WidgetTitle};
    }
    ...
    #Jugni Show Workorder in Zoom page
    my $DBObject           = $Kernel::OM->Get('Kernel::System::DB');
...
WHERE c.id IN
   (SELECT lr.target_key
    ...
    while ( my @Row = $DBObject->FetchrowArray() ) {
        ...
        push @ChangeTitle,  $Row[2];
    }
    my $size = @IDs;
    my @Change;
    my @WorkOrderGraph;
    for ( my $i = 0; $i < $size; $i = $i + 1 ) {
        $Change[$i] = $ChangeObject->ChangeGet(
            ChangeID => $IDs[$i],
            UserID   => $Self->{UserID},
        );

        $WorkOrderGraph[$i] = $LayoutObject->ITSMChangeBuildWorkOrderGraph(
            Change => $Change[$i],
            WorkOrderObject =>
                $Kernel::OM->Get('Kernel::System::ITSMChange::ITSMWorkOrder'),
    ...
    MENU:
        for my $Menu ( sort keys %Menus ) {
            ...
            if ( $Menus{$Menu}->{PopupType} ) {
                $Item->{Class}
                    = "AsPopup PopupType_$Menus{$Menu}->{PopupType}";
            }
     ...
     # within the same cluster wins.
                my $Priority = $MenuClusters{ $Menus{$Menu}->{ClusterName} }
                    ->{Priority};
                if (  !$Priority
                    || $Priority !~ /^\d{3}$/
                ...
                $MenuClusters{ $Menus{$Menu}->{ClusterName} }->{Priority}
                    = $Priority;
                $MenuClusters{ $Menus{$Menu}->{ClusterName} }->{Items}
                    ->{$Menu} = $Item;
            }
        }
        ...
        for my $Item ( sort keys %ZoomMenuItems ) {
            ...
            if ( $ZoomMenuItems{$Item}->{Type} eq 'Cluster' ) {
                ...
                $LayoutObject->Block(
                    ...
                    Data => { Name => $ZoomMenuItems{$Item}->{Name}, },
                );

                for my $SubItem (
                    sort keys %{ $ZoomMenuItems{$Item}->{Items} } )
                {
                    $LayoutObject->Block(
    ...
    if ( $ConfigObject->Get('Ticket::Frontend::MoveType') =~ /^form$/i ) {
        $MoveQueues{0} = '- '
            . $LayoutObject->{LanguageObject}->Translate('Move') . ' -';
    ...
    my %AclActionLookup = reverse %AclAction;
    if ( $ConfigObject->Get('Frontend::Module')->{AgentTicketMove}
        && ( $AclActionLookup{AgentTicketMove} ) )
    {
        ...
        if ($Access) {
            if ( $ConfigObject->Get('Ticket::Frontend::MoveType')
                =~ /^form$/i )
            {
                $LayoutObject->Block(
                    Name => 'MoveLink',
    ...
    if ( $Ticket{CreateBy} > 1 ) {
        $Ticket{CreatedByUser}
            = $UserObject->UserName( UserID => $Ticket{CreateBy} );
        $LayoutObject->Block(
            Name => 'CreatedBy',
            Data => {%Ticket},
        );
    ...
    if ( $ConfigObject->Get('Ticket::Type') ) {
        ...
        my %Type = $Kernel::OM->Get('Kernel::System::Type')
            ->TypeGet( ID => $Ticket{TypeID}, );
    ...
    if ( $ConfigObject->Get('Ticket::Service') && $Ticket{Service} ) {
        ...
        # get service data
        my %Service
            = $Kernel::OM->Get('Kernel::System::Service')->ServiceGet(
            IncidentState => 1,
    ...
    if ( defined $Ticket{FirstResponseTime} ) {
        ...
        $Ticket{FirstResponseTimeWorkingTime}
    ...
    if ( $ConfigObject->Get('Ticket::Frontend::AccountTime') ) {
        $Ticket{TicketTimeUnits}
            = $TicketObject->TicketAccountedTimeGet(%Ticket);
        $LayoutObject->Block(
...
# Enable the video chat feature if system is entitled and agent is a member of configured group.
        if (   $ConfigObject->Get('ChatEngine::Active')
            ...
            && $LayoutObject->{"UserIsGroup[$VideoChatAgentsGroup]"} eq
            'Yes' )
        {
            if ( $Kernel::OM->Get('Kernel::System::Main')
            ...
            {
                $VideoChatEnabled
                    = $Kernel::OM->Get('Kernel::System::VideoChat')
                    ->IsEnabled();
            }
        }
    ...
    FIELD:
        for my $Field (qw(OwnerID ResponsibleID)) {
            ...
            $OnlineData{$Field}->{UserStateDescription}
                ...
                ->Translate('User is currently offline.');

          # We also need to check if the receiving agent has chat permissions.
            my %UserGroups
                = $Kernel::OM->Get('Kernel::System::Group')
                ->PermissionUserGet(
                UserID => $UserID,
                Type   => 'rw',
                );
            ...
            $OnlineData{$Field}->{ChatAccess}
                = $UserGroupsReverse{$ChatReceivingAgentsGroup} ? 1 : 0;

            my %User = $Kernel::OM->Get('Kernel::System::User')
                ->GetUserData( UserID => $UserID, );
            $OnlineData{$Field}->{VideoChatSupport}
                = $User{VideoChatHasWebRTC};

            # Check agent's availability.
            if ( $OnlineData{$Field}->{ChatAccess} ) {
                $OnlineData{$Field}->{AgentChatAvailability}
                    = $Kernel::OM->Get('Kernel::System::Chat')
                ...
                if ( $OnlineData{$Field}->{AgentChatAvailability} == 3 ) {
                    ...
                    $OnlineData{$Field}->{UserStateDescription}
                        = $LayoutObject->{LanguageObject}
                        ->Translate('User is currently active.');
                    ...
                    $OnlineData{$Field}->{UserStateDescription}
                        = $LayoutObject->{LanguageObject}
                        ->Translate('User was inactive for a while.');
                }
                elsif ( $OnlineData{$Field}->{AgentChatAvailability} == 1 ) {
                    $OnlineData{$Field}->{UserState}
                        = Translatable('Unavailable');
                    $OnlineData{$Field}->{UserStateDescription}
                        = $LayoutObject->{LanguageObject}
                        ->Translate('User set their status to unavailable.');
                }
            }
        }
    }
    ...
    if ( $ConfigObject->Get('Ticket::Responsible') ) {
        $LayoutObject->Block(
            Name => 'Responsible',
            Data => {
                %Ticket, %ResponsibleInfo,
                %AclAction, %{ $OnlineData{ResponsibleID} // {} }
            },
        );
    }
    ...
    if ( !@ArticleBox && !$Self->{ZoomTimeline} ) {
        $LayoutObject->Block( Name => 'HintNoArticles', );
    }

    # show process widget  and activity dialogs on process tickets
    # ~GS
    my $CounterForProcess = 0;
    my $flag              = 'false';

    if ($IsProcessTicket) {

        # ~GS
        $CounterForProcess = $CounterForProcess + 1;
        ...
        my $ProcessData
            = $Kernel::OM->Get('Kernel::System::ProcessManagement::Process')
            ->ProcessGet( ProcessEntityID => $Ticket{$ProcessEntityIDField},
            );
        my $ActivityData
            = $Kernel::OM->Get('Kernel::System::ProcessManagement::Activity')
            ->ActivityGet(
        ...
        $LayoutObject->Block(
            Name => 'ProcessWidget',
            Data => { WidgetTitle => $Param{WidgetTitle}, },
        );
        ...
        if ($NextActivityDialogs) {
            ...
            my $ActivityDialogObject = $Kernel::OM->Get(
...
# and display/hide the activity dialog link
        ...
        ACTIVITYDIALOGPERMISSION:
            ...
            {
                my $CurrentActivityDialogEntityID
                    = $NextActivityDialogs->{$Index};
                my $CurrentActivityDialog
                    = $ActivityDialogObject->ActivityDialogGet(
                    Interface              => 'AgentInterface',
                    ActivityDialogEntityID => $CurrentActivityDialogEntityID
                    );

                # create an interface lookup-list
                my %InterfaceLookup = map { $_ => 1 }
                    @{ $CurrentActivityDialog->{Interface} };

                next ACTIVITYDIALOGPERMISSION
                    if !$InterfaceLookup{AgentInterface};

                if ( $CurrentActivityDialog->{Permission} ) {

                    # performance-boost/cache
                    if (!defined $PermissionRights{ $CurrentActivityDialog
                                ->{Permission} } )
                    {
                        $PermissionRights{ $CurrentActivityDialog
                                ->{Permission} }
                    ...
                    if (!$PermissionRights{ $CurrentActivityDialog
                                ->{Permission} } )
                    {
                        next ACTIVITYDIALOGPERMISSION;
                    }
...
#Author:Tuere
#Date:20/07/2017
# If ADPermission = 2; check if current USerID = Ticket->ResponsibleID;if not responsible = current User, go to next AD

                if (   $Ticket{DynamicField_ADPermission}
                    && $Ticket{DynamicField_ADPermission} == 2 )
                {
                ...
                $PermissionActivityDialogList{$Index}
                    = $CurrentActivityDialogEntityID;
            }
            ...
            if ( IsHashRefWithData($NextActivityDialogs) ) {
                for my $NextActivityDialogKey (
                    sort { $a <=> $b }
                    keys %{$NextActivityDialogs}
                    )
                {
                    ...
                    $LayoutObject->Block(
                        Name => 'ActivityDialog',
                        Data => {
                            ActivityDialogEntityID => $NextActivityDialogs
                                ->{$NextActivityDialogKey},
    ...
    if ( $flag eq 'false' ) {
        if ( $CounterForProcess > 0 ) {
            $LayoutObject->Block(
                Name => 'NoProcessFlow',
                Data => { 'message' => 'No Further Actions Available', },
            );
        }
        else {
            $LayoutObject->Block(
                Name => 'ProcessWidget',
                Data => { WidgetTitle => 'Process Flow', },
            );

            $LayoutObject->Block(
                Name => 'NoProcessFlow',
                Data => { 'message' => 'No Process Flow Attached', },
            );
        }
    }
...
DYNAMICFIELD:
    for my $DynamicFieldConfig ( @{$DynamicField} ) {
        ...
        next DYNAMICFIELD
            if $Ticket{ 'DynamicField_' . $DynamicFieldConfig->{Name} } eq '';
...
# use translation here to be able to reduce the character length in the template
        my $Label = $LayoutObject->{LanguageObject}
            ->Translate( $DynamicFieldConfig->{Label} );

        if (   $IsProcessTicket
            && $Self->{DisplaySettings}->{ProcessWidgetDynamicField}
    ...
    if ($IsProcessTicket) {
        ...
        for my $GroupName (
            sort keys
            %{ $Self->{DisplaySettings}->{ProcessWidgetDynamicFieldGroups} } )
        {

            $LayoutObject->Block( Name => 'ProcessWidgetDynamicFieldGroups',
            );
            ...
            if ( $#GroupFields + 1 ) {
                ...
                for my $Field (@FieldsWidget) {
                    ...
                    if ( grep { $_ eq $Field->{Name} } @GroupFields ) {
                        ...
                        $LayoutObject->Block( Name =>
                                'ProcessWidgetDynamicFieldValueOverlayTrigger',
                        );
       ...
       # output dynamic fields not registered in a group in the process widget
        ...
        if ( $#RemainingFieldsWidget + 1 ) {
            ...
            $LayoutObject->Block(
                Name => 'ProcessWidgetDynamicFieldGroupSeparator',
                Data => {
                    Name => $LayoutObject->{LanguageObject}
                        ->Translate('Fields with no group'),
                },
            );
        }
...
DYNAMICFIELD:
    ...
    if ( $ConfigObject->Get('Ticket::Frontend::CustomerInfoZoom') ) {
        ...
        $Param{CustomerTable} = $LayoutObject->AgentCustomerViewTable(
            Data   => \%CustomerData,
            Ticket => \%Ticket,
            Max    => $ConfigObject->Get(
                'Ticket::Frontend::CustomerInfoZoomMaxSize'),
        );

        # Block of code added by Geeta Suryawanshi ~GS
        if ( $Param{CustomerTable} eq 'none' ) {
            $Param{CustomerTable} = 'No Data Found';
        }
...
sub _ArticleTree {
    ...
    if ( $Self->{ArticleFilterActive} ) {
        ...
        $LayoutObject->Block(
            ...
            Data => { %Param, HighlightStyle => $HighlightStyle, },
        );
    ...
    if ( !$Self->{ZoomTimeline} ) {
        ...
        $LayoutObject->Block(
            Name => 'ArticleList',
            Data => { %Param, TableClasses => $TableClasses, },
        );
    ...
    ARTICLE:
        for my $ArticleTmp (@ArticleBox) {
            ...
            if ( $Self->{ArticleFilterActive} ) {
                    ...
                    && $Self->{ArticleFilter}->{ShownArticleIDs} )
                {
                 ...
                 # do not show article in tree if it does not match the filter
                    if ( !$Self->{ArticleFilter}->{ShownArticleIDs}
                        ->{ $Article{ArticleID} } )
                    {
                        next ARTICLE;
                    }
                ...
                )
            {
                ...
                my $ShowMeta;
                if (   $Self->{UserID} == $Article{OwnerID}
...
# Always use user id 1 because other users also have to see the important flag
            ...
            if ( $Article{ArticleType} =~ /-internal$/smx ) {
                ...
                if ( $Article{SenderType} eq 'customer' ) {
                    $LayoutObject->Block(
                        Name => 'TreeItemDirectionIncoming' );
                }
                else {
                    $LayoutObject->Block(
                        Name => 'TreeItemDirectionOutgoing' );
                }
            }
            ...
            if ( !$Article{Atms} || !%{ $Article{Atms} } ) {
                ...
                my $Attachments
                    = $Self->_CollectArticleAttachments( Article => \%Article,
                    );
    ...
    # show timeline view
        ...
        my @TypesLeft
            = ( @TypesOutgoing, @TypesInternal, @TypesTicketAutoAction, );

        my @TypesRight = ( @TypesIncoming, @TypesTicketAction, );

        my @TypesWithArticles = (
            @TypesOutgoing, @TypesInternal,
            @TypesIncoming, 'PhoneCallCustomer',
        );
        ...
        my %HistoryTypes;
        my %HistoryTypeConfig = %{ $Kernel::OM->Get('Kernel::Config')
                ->Get('Ticket::Frontend::HistoryTypes') // {} };
        for my $Entry ( sort keys %HistoryTypeConfig ) {
            %HistoryTypes
                = ( %HistoryTypes, %{ $HistoryTypeConfig{$Entry} }, );
        }
...
# if the 'NewTicket' item has an article, display this "creation article" event separately
     ...
     # special treatment for certain types, e.g. external notes from customers
                ...
                && IsHashRefWithData(
                    $ArticlesByArticleID->{ $Item->{ArticleID} }
                )
                && $ArticlesByArticleID->{ $Item->{ArticleID} }->{SenderType}
                eq 'customer'
                )
            {
   ...
   # have the history type 'AddNote' which does not allow for distinguishing.
            ...
            # special treatment for internal emails
                ...
                && $ArticlesByArticleID->{ $Item->{ArticleID} }->{ArticleType}
                eq 'email-internal'
                )
            {
     ...
     # special treatment for certain types, e.g. external notes from customers
            ...
            elsif ( grep { $_ eq $Item->{HistoryType} }
     ...
     # remove article information from types which should not display articles
           ...
           # display only the first three (shortened) lines of a chart article
                    my $ChatMessages
                        ...
                        ->Decode( Data => $Item->{ArticleData}->{Body}, );

                    for my $ChatMessage ( @{ $ChatMessages // [] } ) {
                        $ChatMessage->{CreateTime}
                            = $LayoutObject->{LanguageObject}
                            ->FormatTimeString( $ChatMessage->{CreateTime},
                            'DateFormat' );
                    }
                ...
                CHATITEM:
                    for my $MessageData ( sort { $a->{ID} <=> $b->{ID} }
                        @{$ChatMessages} )
                    {
                        if ( $MessageData->{SystemGenerated} == 1 ) {

                            $Item->{ArticleData}->{BodyChat}
                                .= $LayoutObject->Output(
                                Template =>
                ...
                if ( $Item->{Name} && $Item->{Name} =~ m/^%%/x ) {
                    ...
                    $Item->{Name}
                        = $LayoutObject->{LanguageObject}
                        ->Translate( $HistoryTypes{ $Item->{HistoryType} },
                        @Values, );
            ...
            $Item->{CreateSystemTime}
                = $Kernel::OM->Get('Kernel::System::Time')
                ->TimeStamp2SystemTime( String => $Item->{CreateTime} );
      ...
      # them as if they happened exactly on the same time (treshold 5 seconds)
            if (   $LastCreateSystemTime
                && $Item->{CreateSystemTime} <= $LastCreateSystemTime
                && $Item->{CreateSystemTime}
                >= ( $LastCreateSystemTime - 5 ) )
            {
        ...
        for my $ArticleID ( sort keys %{$ArticlesByArticleID} ) {
            ...
            if (IsHashRefWithData(
                    $ArticlesByArticleID->{$ArticleID}->{Atms}
                )
                )
            {
...
sub _TicketItemSeen {
    ...
    my @ArticleIDs = $Kernel::OM->Get('Kernel::System::Ticket')
        ->ArticleIndex( TicketID => $Param{TicketID}, );

    for my $ArticleID (@ArticleIDs) {
        $Self->_ArticleItemSeen( ArticleID => $ArticleID, );
    }
...
sub _ArticleItem {
    ...
    my @MenuItems = $Self->_ArticleMenu( %Param, );

    # get layout object
    my $LayoutObject = $Kernel::OM->Get('Kernel::Output::HTML::Layout');

    # collect article meta
    my @ArticleMetaData = $Self->_ArticleCollectMeta( Article => \%Article );
    ...
    if ( $Article{CreatedBy} > 1 ) {
        $Article{CreatedByUser} = $Kernel::OM->Get('Kernel::System::User')
            ->UserName( UserID => $Article{CreatedBy} );
        $LayoutObject->Block(
    ...
    my $RecipientDisplayType
        = $ConfigObject->Get('Ticket::Frontend::DefaultRecipientDisplayType')
        || 'Realname';
    my $SenderDisplayType
        = $ConfigObject->Get('Ticket::Frontend::DefaultSenderDisplayType')
        || 'Realname';
KEY:
    for my $Key (qw(From To Cc)) {
        ...
        my $DisplayType
            = $Key eq 'From' ? $SenderDisplayType : $RecipientDisplayType;
...
DYNAMICFIELD:
    ...
    if ( $Article{Atms} && %{ $Article{Atms} } ) {
        ...
        my $Config
            = $ConfigObject->Get('Ticket::Frontend::ArticleAttachmentModule');
    ATTACHMENT:
    ...
    if (   $Article{ArticleType} eq 'chat-external'
        || $Article{ArticleType} eq 'chat-internal' )
    {

        $LayoutObject->Block(
            Name => 'BodyChat',
            Data => {
                ChatMessages => $Kernel::OM->Get('Kernel::System::JSON')
                    ->Decode( Data => $Article{Body}, ),
            },
        );
...
sub _ArticleMenu {
    ...
    if ( $Article{ArticleType} !~ /^(note|email-noti|chat)/i ) {
            ...
            && ( $AclActionLookup{AgentTicketCompose} ) )
        {
            my $Access = 1;
            my $Config
                = $ConfigObject->Get('Ticket::Frontend::AgentTicketCompose');
            ...
            if ($Access) {
                ...
                my %StandardResponseHash
                    = %{ $Param{StandardResponses} || {} };
...
# and @StandardResponseArray is created as array of hashes with elements Key and Value
                ...
                my %ReverseStandardResponseHash
                    = reverse %StandardResponseHash;
            ...
            KEY:
                ...
                if ($Recipients) {
                    my $EmailParser
                        = Kernel::System::EmailParser->new( %{$Self},
                        Mode => 'Standalone', );
                    my @Addresses = $EmailParser->SplitAddressLine(
                        Line => $Recipients );
                ADDRESS:
                    for my $Address (@Addresses) {
                        my $Email = $EmailParser->GetEmailAddress(
                            Email => $Address );
                        next ADDRESS if !$Email;
                        my $IsLocal
                            = $Kernel::OM->Get(
                            'Kernel::System::SystemAddress')
                            ->SystemAddressIsLocalAddress( Address => $Email,
                            );
...
# (only show forward on email-external, email-internal, phone, webrequest and fax
        if (   $ConfigObject->Get('Frontend::Module')->{AgentTicketForward}
            && $AclActionLookup{AgentTicketForward}
            && $Article{ArticleType}
            =~ /^(email-external|email-internal|phone|webrequest|fax)$/i )
        {
            my $Access = 1;
            my $Config
                = $ConfigObject->Get('Ticket::Frontend::AgentTicketForward');
            if ( $Config->{Permission} ) {
                my $OK = $TicketObject->TicketPermission(
                    Type     => $Config->{Permission},
                    TicketID => $Ticket{TicketID},
...
# and @StandardForwardArray is created as array of hashes with elements Key and Value
                    ...
                    push @MenuItems,
                        ...
                        ItemType => 'Link',
                        Description =>
                            Translatable('Forward article via mail'),
                        Name  => Translatable('Forward'),
                        Class => 'AsPopup PopupType_TicketAction',
                        Link =>
        ...
        if (   $ConfigObject->Get('Frontend::Module')->{AgentTicketBounce}
            && $AclActionLookup{AgentTicketBounce}
            && $Article{ArticleType} =~ /^(email-external|email-internal)$/i )
        {
            my $Access = 1;
            my $Config
                = $ConfigObject->Get('Ticket::Frontend::AgentTicketBounce');
            if ( $Config->{Permission} ) {
                my $OK = $TicketObject->TicketPermission(
                    Type     => $Config->{Permission},
                    TicketID => $Ticket{TicketID},
    ...
    if (   $ConfigObject->Get('Frontend::Module')->{AgentTicketPrint}
    ...
    if (   $ConfigObject->Get('Frontend::Module')->{AgentTicketPlain}
        ...
        && $Article{ArticleType} =~ /email/i )
    {
        ...
        if ($OK) {
            ...
            push @MenuItems,
                ...
                ItemType => 'Link',
                Description =>
                    Translatable('View the source for this Article'),
...
# Always use user id 1 because other users also have to see the important flag
    ...
    if (   $ConfigObject->Get('Frontend::Module')->{AgentTicketNote}
...
sub _ArticleCollectMeta {
    ...
    my %Config
        = %{ $ConfigObject->Get('Ticket::Frontend::ZoomCollectMetaFilters') };
        ...
        if ( scalar @Matches ) {
            ...
            for my $Match (@Matches) {
                ...
                for my $Part ( sort keys %{ $Match->{Parts} || {} } ) {
                    $MatchLinkEncode = $LayoutObject->LinkEncode(
                        $Match->{Parts}->{$Part} );
