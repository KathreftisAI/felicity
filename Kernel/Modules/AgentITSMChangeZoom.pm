...
sub Run {
    ...
    return if !$DBObject->Prepare(
       SQL => 'SELECT id, change_id, workorder_state_id, workorder_agent_id FROM change_workorder WHERE change_id =?',
       Bind => [\$ChangeID],
    );
    ...
    while (my @RowsofWOs = $DBObject->FetchrowArray()){
      ...
      my $temp_woagent_fullname = %{temp_woagent}->{'UserFullname'};
       ...
       # push @WOIDs,$RowsofWOs[0];
      # push @WOSIDs,$RowsofWOs[2];
      #push @WOAIDs,$RowsofWOs[3];
    }

    %WODisplay->{"ChangeID"} = $ChangeID;
    $WODisplay{"WorkOrderArray"} = \@{WOOrders};
    $WODisplay{"WorkOrderAgentArray"} = \@{WOOrderAgent};
    #%WODisplay->{"WorkorderIDs"} = @WOIDs;
    #%WODisplay->{"StateIDs"} = @WOSIDs;
    #%WODisplay->{"AgentIDs"} = @WOAIDs;
