package Kernel::Modules::AgentBulkPrinter;
...
sub Run {
    ...
    my $BulkTicketID=$ParamObject->GetParam( Param => 'BulkTicketID' );
         # check needed stuff
     if ( !$BulkTicketID ) {
        return $LayoutObject->ErrorScreen(
            Message => Translatable('Need BulkTicketID!'),
            );
     }



    my @BulkTicketIDs= split(/,/,$BulkTicketID );
    my $Output;
       
        

    for my $BKeys (0 .. $#BulkTicketIDs){

        $Self->{TicketID} =$BulkTicketIDs[$BKeys];  
        my %TicketData;
        ...
        #     my $LinkTypeName = $LinkTypeList{ $LinkData[0] }->{ $LinkData[1] . 'Name' };
        #     $LinkTypeName = $LayoutObject->{LanguageObject}->Translate($LinkTypeName);

        #     my %LinkType;
        #     $LinkType{Namee} = $LinkTypeName;
        ...
        $TicketData{PrintedBy}= "$PrintedBy $Self->{UserFirstname}$Self->{UserLastname} ($Self->{UserEmail}), $Time";
        ...
        $Param{Table}=%table;
        @TicketData{keys %table} = values %table;
        @TicketData{keys %CustomerData} = values %CustomerData; 
        ...
        $Param{ArtileTree} = $Self->_ArticleList(
...
# ---
    ...
    for my $Row ( 1 .. $Rows ) {
        ...
        if($TableRight->[$Row]{Value}){
            $RightTable{$TableRight->[$Row]{Key}}=$TableRight->[$Row]{Value};           
        }
    }
