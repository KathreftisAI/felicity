...
package Kernel::Modules::CustomerTicketMessageNew;
...
#added below code for set the service based on config value
	if ( $Config->{'ServiceDefault'} && !$Param{ServiceID} ) {
            my %ReverseType = reverse %Services;
            $Param{ServiceID} = $ReverseType{ $Config->{'ServiceDefault'} };
        }
#End of code
    ...
    return $LayoutObject->Output(
        TemplateFile => 'CustomerTicketMessageNew',
        Data         => \%Param,
    );
}
