...
package Kernel::Modules::CMDBGlobal;
...
"Select id,session_id,token FROM sso_servers_tokens WHERE session_id LIKE ?";
    ...
    if ( !$TokenData{Token} ) {
        ...
        my $query =
"Select id,server_ip FROM sso_servers WHERE server_name LIKE 'glpiurl'";
        $DBObject->Prepare(
            SQL   => $query,
            Limit => 1,
        );
    ...
    $mydata{'url'} =
      $glpiurl . "allassets.php?embed=felicity" . "&token=" . $Token;
    $mydata{'title'} = "CMDB Software";
