...
package Kernel::Modules::AgentNewEscalation;
...
sub Run {
    my ( $Self, %Param ) = @_;
	#my @ViewableTicketsTypeIDS =(3);
#Jugni added below code for get the ID based on Incident from Type DB.
	my @ViewableTicketsTypeIDS;
	...
	( $Sec, $Min, $Hour, $Day, $Month, $Year ) = $TimeObject->SystemTime2Date(
        SystemTime => $TimeObject->SystemTime() + 60 * 30,
    );
    my $TimeStampNext30Mins = "$Year-$Month-$Day 23:59:59";
    ...
    my %Filters = (
        Today => {
            ...
            Search => {
				...
				# TypeIDs                       => \@ViewableTicketsTypeIDS,
            },
        },
        Tomorrow => {
		...
		Next30Mins => {
            Name   => Translatable('Next 30 Mins'),
            Prio   => 4000,
            Search => {
                TicketEscalationTimeOlderDate => $TimeStampNext30Mins,
                OrderBy                       => $OrderBy,
                SortBy                        => $SortBy,
                UserID                        => $Self->{UserID},
                Permission                    => $Config->{'TicketPermission'},
    ...
    {
		#############################Filter for Incident Viewable Tickets ###########################################3 
	...
	####################### Filter for Today , Tomorrow, Weekly Navigation bar display numbers. ###################################333
    my %NavBarFilter;
	...
	# open my $fileHandle, ">>", "$existingdir/IncidentEscalation.txt" or die "Can't open '$existingdir/IncidentEscalation.txt'\n";
