...
package Kernel::Modules::MyNewModule;
...
sub Run {
    ...
    # ------------------------------------------------------------ #
        ...
        my $Note = '';
        my ( %GetParam, %Errors );
        for my $Parameter (qw(mainquery subquery userid)) {
            $GetParam{$Parameter} = $ParamObject->GetParam( Param => $Parameter ) || '';
        }

        # check needed data
        for my $Needed (qw(mainquery subquery userid)) {
            if ( !$GetParam{$Needed} ) {
                $Errors{ $Needed . 'Invalid' } = 'ServerError';
            }
        }

        my %Data = $Self->DynamicFieldExtendGet( userid => $GetParam{userid} );
        if ( !%Data ) {
            return $LayoutObject->ErrorScreen(
