...
package Kernel::Modules::RequestEscalation;
...
sub Run {
    my ( $Self, %Param ) = @_;
    #my @ViewableTicketsTypeIDS =(5);

#Jugni added below code for get the request id based on Type DB.
    ...
    my %GetColumnFilter;
    COLUMNNAME:
	############################Removed Type from the Header List ################################################################33
    for my $ColumnName (
    ...
    {
######################## Hides all the types except one in filter Type Dropdown #########################################3		
...
#############################Filter for Request Viewable Tickets ###########################################3 
